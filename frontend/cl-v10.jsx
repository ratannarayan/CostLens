...
const SC_CATS=[{key:"rawMaterial",label:"Raw Material",color:"#3b82f6"},{key:"processing",label:"Manufacturing / Processing",color:"#06b6d4"},{key:"secondary",label:"Secondary / Finishing",color:"#14b8a6"},{key:"overheads",label:"Overheads (15% on Process)",color:"#f59e0b"},{key:"profit",label:"Profit (8%)",color:"#ef4444"}];
...
// ─── LOGO SVG COMPONENT ───
const CLIcon=({size=24,white})=>{const s=size/48;return <svg width={size} height={size} viewBox="0 0 48 48" style={{display:"inline-block",verticalAlign:"middle",flexShrink:0}}><circle cx="20" cy="22" r="14" fill="none" stroke={white?"#fff":"#1B4F72"} strokeWidth="2.4"/><line x1="30" y1="32" x2="40" y2="42" stroke={white?"#fff":"#1B4F72"} strokeWidth="2.4" strokeLinecap="round"/><rect x="12" y="16" width="5" height="11" rx="1.2" fill="#2E86C1" opacity=".9"/><rect x="18" y="20" width="5" height="7" rx="1.2" fill="#10B981" opacity=".9"/><rect x="24" y="14" width="5" height="13" rx="1.2" fill="#F59E0B" opacity=".9"/></svg>};
const CLLogo=({size=18,white,gap=6,sub})=><div style={{display:"flex",alignItems:"center",gap:gap}}><CLIcon size={size*1.4} white={white}/><div><span style={{fontFamily:"'Bricolage Grotesque',sans-serif",fontSize:size,fontWeight:800,color:white?"#fff":"#1B4F72"}}>Cost</span><span style={{fontFamily:"'Bricolage Grotesque',sans-serif",fontSize:size,fontWeight:800,color:"#2E86C1"}}>Lens</span>{sub&&<div style={{fontSize:size*0.4,fontWeight:600,letterSpacing:1.5,color:white?"rgba(255,255,255,.5)":"#999",textTransform:"uppercase",marginTop:-1}}>Procurement Intelligence</div>}</div></div>;
...
const EBOOK_TOC={
  1:["Why Most Cost Reduction Programs Fail","Setting the Foundation: Baseline & Benchmarking","The 5-Lever Framework: Negotiate, Consolidate, Substitute, Redesign, Eliminate","Building Your Savings Pipeline (3× Rule)","Wave Planning: Quick Wins → Structural → Transformational","Cross-Functional War Rooms That Work","Tracking & Validating: From Claimed to P&L-Confirmed","Sustaining Gains: How to Prevent Cost Creep","Templates: Initiative Tracker, Savings Waterfall, Monthly Review Deck"],
  2:["Category Management vs Category Buying","Spend Segmentation: The ABC-Kraljic Hybrid","Supply Market Analysis Frameworks","Strategy Formulation: 7 Sourcing Levers per Quadrant","Category Playbooks That Teams Actually Use","Cross-Functional Category Teams: RACI & Governance","Implementation Roadmap: 90-Day Quick Start","Measuring Category Maturity (1-5 Scale)","Templates: Category Profile, Strategy Canvas, QBR Deck"],
  3:["The Real Cost of Supply Chain Disruption","Risk Identification: 6 Dimensions of Supply Risk","Risk Assessment: Probability × Impact Matrix","Supply Chain Mapping: Tier 1, 2 & Beyond","Building Dual/Multi-Source Strategies","Inventory as a Risk Buffer: How Much is Enough?","Supplier Financial Health Monitoring","Business Continuity Planning with Key Suppliers","Templates: Risk Register, BCP Template, Supplier Health Dashboard"],
  4:["Anatomy of a Should-Cost Model","Raw Material Costing: Weight × Rate × Scrap Factor","Manufacturing Process Sheets: Time Studies & Machine Rates","Secondary Operations & Finishing Costs","Overhead Allocation Methods (15-25% Range)","Profit Margin Benchmarks by Industry","Volume Discount Modeling: The Learning Curve Effect","Using Should-Cost in Negotiations: The Data Advantage","Templates: Process Sheet, RM Calculator, Negotiation Prep Sheet"],
  5:["Strategic Sourcing vs Tactical Buying","Step 1: Category Profiling & Spend Analysis","Step 2: Supply Market Assessment","Step 3: Sourcing Strategy Development","Step 4: RFx Process Design & Execution","Step 5: Negotiation & Supplier Selection","Step 6: Contract & Implementation","Step 7: Performance Management & Review","Templates: RFQ Scorecard, Negotiation Planner, Supplier Evaluation Matrix"],
  6:["Hard Savings vs Soft Savings vs Cost Avoidance","Setting Baselines: Last Transacted Price, Not Budget","Savings Calculation Methodologies (4 Approved Methods)","Avoiding Double-Counting: Unique Initiative IDs","The Validation Process: Procurement → Finance → CFO","P&L Mapping: Which Line Item Gets the Credit?","Building a Savings Dashboard That CFOs Trust","Common Pitfalls: Budget-Based, Annualization Errors","Templates: Savings Tracker, Validation Checklist, CFO Report Format"],
  7:["Why Risk Assessment is a Procurement Responsibility","The 6-Dimension Risk Framework","Single-Source Risk: Identification & Mitigation","Geographic Concentration Analysis","Supplier Financial Risk Indicators","Operational Risk: Quality, Delivery, Compliance","Building the Heat Map: Severity × Likelihood","Mitigation Strategies by Risk Quadrant","Templates: Risk Register, Heat Map, Mitigation Action Plan"],
  8:["The Case for Procurement Transformation","Maturity Assessment: Where Are You Today? (5 Levels)","Building the Vision: From Cost Center to Value Creator","Stakeholder Management: Winning Hearts & Minds","Quick Wins That Build Credibility (First 90 Days)","Technology Enablement: ERP, Analytics, AI","Organization Design: Skills, Structure, Roles","Change Management: The People Side of Transformation","Templates: Maturity Scorecard, Transformation Roadmap, Stakeholder Map"],
  9:["Why Measure Supplier Performance?","The Composite Scorecard: Q40-D30-C20-R10","Quality Metrics: PPM, Rejection Rate, COPQ","Delivery Metrics: OTIF%, Lead Time Adherence","Cost Metrics: Price Competitiveness, YoY Trend","Responsiveness: Quote Turnaround, Issue Resolution","Supplier Segmentation: Green, Amber, Red Actions","QBR Best Practices: Data → Discussion → Action","Templates: Scorecard Template, QBR Agenda, Development Plan"],
  10:["What Is Spend Analysis & Why It Matters","Data Collection: ERP Extraction Best Practices","Data Cleansing: Supplier Name Normalization","Spend Classification: UNSPSC & Custom Taxonomies","Pareto Analysis: The 80/20 of Procurement","Maverick Spend Detection & Elimination","Tail Spend Management Strategies","Turning Insights into Action: Savings Identification","Templates: Data Extraction Guide, Classification Matrix, Savings Heatmap"]
};

// ─── ANALYSIS REPORTS ───
const REPORTS=[
  {id:"spend",icon:"🔍",name:"Spend Analysis",desc:"Classify, segment, and find savings opportunities in your procurement spend data",credits:3,expertPrice:"₹15,000–25,000",turnaround:"24-48 hrs",
  ...
  {id:"price-variance",icon:"📉",name:"Price Variance Report",desc:"Track price changes across suppliers, materials, and periods — find who's charging more",credits:2,expertPrice:"₹8,000–12,000",turnaround:"24 hrs",
  ...
  {id:"inventory-health",icon:"📦",name:"Inventory Health Analysis",desc:"Aging analysis, NMS identification, turnover ratios, and liquidation planning",credits:2,expertPrice:"₹10,000–15,000",turnaround:"24-48 hrs",
  ...
  {id:"supplier-scorecard",icon:"⭐",name:"Supplier Performance Scorecard",desc:"Quality, delivery, cost, and responsiveness scoring across your supply base",credits:2,expertPrice:"₹8,000–12,000",turnaround:"24 hrs",
  ...
  {id:"category-opportunity",icon:"🎯",name:"Category Opportunity Analysis",desc:"Strategic assessment of procurement categories to identify and prioritize savings levers",credits:3,expertPrice:"₹15,000–20,000",turnaround:"48 hrs",
  ...
  {id:"cost-reduction-tracker",icon:"📋",name:"Cost Reduction Pipeline Tracker",desc:"Track, validate, and report savings initiatives across all procurement levers",credits:2,expertPrice:"₹10,000–15,000",turnaround:"24-48 hrs",
  ...
  {id:"savings-validation",icon:"✅",name:"Procurement Savings Validation",desc:"CFO-ready savings report with P&L mapping, methodology validation, and audit trail",credits:3,expertPrice:"₹12,000–18,000",turnaround:"48 hrs",
  ...
  {id:"supplier-risk",icon:"⚠️",name:"Supplier Risk Heat Map",desc:"Identify single-source, geographic, financial, and operational risks across your supply base",credits:2,expertPrice:"₹8,000–12,000",turnaround:"24 hrs",
...
const AI_TOOLS=[
  {id:"price-check",icon:"💲",name:"Price Reasonableness Check",desc:"Instant AI check — is this quotation price fair?",credits:1,
    fields:[{k:"itemDesc",l:"Item Description",ph:"CNC machined aluminum housing, 2.3kg"},{k:"material",l:"Material Type",ph:"Aluminum 6061-T6"},{k:"weight",l:"Weight/Size",ph:"2.3 kg"},{k:"process",l:"Manufacturing Process",ph:"CNC machining + anodizing"},{k:"supplierPrice",l:"Supplier Quoted Price (₹)",t:"number"},{k:"quantity",l:"Annual Quantity",t:"number"},{k:"location",l:"Supplier Location",ph:"Pune, Maharashtra"}]},
  {id:"contract-analyzer",icon:"📜",name:"Contract Clause Analyzer",desc:"Upload supplier contract → AI flags risky, missing, and favorable clauses",credits:3,
    needsFile:true,fileHint:"Upload supplier contract / agreement PDF (1-50 pages)"},
  {id:"rfq-comparator",icon:"📊",name:"RFQ Response Comparator",desc:"Upload 2-5 supplier quotations → Normalized comparison matrix",credits:3,
    needsFile:true,multiFile:true,fileHint:"Upload 2-5 supplier quotation PDFs or Excel files"},
  {id:"negotiation-brief",icon:"🎯",name:"Negotiation Prep Brief",desc:"AI-generated 1-page strategy brief for your next supplier negotiation",credits:2,
    fields:[{k:"supplierName",l:"Supplier Name",ph:"Tata Steel"},{k:"items",l:"Items Being Negotiated",ph:"HR Coil, CR Sheets, GP Coils"},{k:"currentSpend",l:"Current Annual Spend (₹ Lakhs)",t:"number"},{k:"contractType",l:"Contract Status",t:"select",opts:["New Contract","Annual Renewal","Price Revision","Spot Negotiation","Dispute Resolution"]},{k:"objective",l:"Your Primary Objective",t:"select",opts:["Cost Reduction (5-10%)","Cost Reduction (10-15%)","Payment Terms Improvement","Quality Improvement","Delivery Improvement","Dual Sourcing / De-risking","Volume Commitment Deal"]},{k:"leverage",l:"Your Known Leverage Points",ph:"2 alternate suppliers qualified, commodity index dropped 8%, volume increased 20%"},{k:"constraints",l:"Constraints / Red Lines",ph:"Cannot change spec, must maintain 60-day payment terms"},{k:"history",l:"Relationship History",ph:"5-year supplier, good quality, prices above market by 8%"}]},
];

const TOOL_PROMPTS={
"price-check":(f)=>`You are a senior procurement cost engineer with 25+ years in Indian manufacturing. Analyze this quotation for price reasonableness.

ITEM: ${f.itemDesc||"Not specified"}
MATERIAL: ${f.material||"Not specified"}
WEIGHT/SIZE: ${f.weight||"Not specified"}
PROCESS: ${f.process||"Not specified"}
SUPPLIER QUOTED PRICE: ₹${f.supplierPrice||0}
ANNUAL QUANTITY: ${f.quantity||"Not specified"}
SUPPLIER LOCATION: ${f.location||"Not specified"}

Assess whether this price is reasonable based on:
1. Raw material cost estimate (weight × market rate for this material in India)
2. Conversion cost estimate (process complexity, cycle time assumptions)
3. Overhead, logistics, profit margin typical for this type of supplier
4. Volume impact on pricing
5. Regional cost differences

RESPOND ONLY VALID JSON:
{"verdict":"Reasonable|High|Very High|Low|Very Low","confidence":"High|Medium|Low","quotedPrice":0,"estimatedFairRange":{"low":0,"high":0},"breakdown":{"material":{"estimate":"₹X","basis":"reasoning"},"conversion":{"estimate":"₹X","basis":"reasoning"},"overhead":{"estimate":"₹X","basis":"reasoning"},"logistics":{"estimate":"₹X","basis":"reasoning"},"profit":{"estimate":"₹X","basis":"reasoning"}},"keyObservations":["observation1"],"suggestedActions":["action1"],"negotiationTip":"one key leverage point"}`,

"contract-analyzer":`You are a senior procurement contracts specialist with deep expertise in Indian commercial law, supply agreements, and manufacturing procurement contracts.

Analyze the uploaded contract document thoroughly. Extract and evaluate EVERY commercial clause.

RESPOND ONLY VALID JSON:
{"contractSummary":{"parties":"","type":"","tenure":"","value":"","effectiveDate":"","expiryDate":""},"favorableClauses":[{"clause":"clause name","section":"section ref","detail":"what it says","benefit":"why it's good for buyer"}],"riskyAndUnfavorableClauses":[{"clause":"clause name","section":"section ref","detail":"what it says","risk":"specific risk to buyer","severity":"High|Medium|Low","amendment":"suggested rewording"}],"missingCriticalClauses":[{"clause":"missing clause name","importance":"High|Medium","risk":"what could go wrong","suggestedText":"recommended clause language"}],"commercialTermsSummary":{"paymentTerms":"","priceRevision":"","ldClause":"","warranty":"","terminationClause":"","forceMultiple":"","ipOwnership":"","confidentiality":"","disputeResolution":"","insurance":"","indemnity":""},"overallRiskScore":"Low|Medium|High|Critical","topRecommendations":["recommendation1"]}`,

"rfq-comparator":`You are a senior procurement analyst expert in techno-commercial evaluation of supplier quotations in Indian manufacturing.

Analyze ALL uploaded supplier quotations. Create a normalized comparison across all vendors.

RESPOND ONLY VALID JSON:
{"rfqSummary":{"item":"","totalVendors":0,"evaluationDate":""},"vendors":[{"name":"","unitPrice":0,"currency":"INR","moq":"","leadTimeDays":0,"paymentTerms":"","warranty":"","deliveryTerms":"","validityDays":0,"keyDeviations":["deviation"],"strengths":["strength"],"weaknesses":["weakness"]}],"comparisonMatrix":[{"parameter":"","weight":0,"vendors":{"vendorName":"score/detail"}}],"priceAnalysis":{"lowestPrice":0,"highestPrice":0,"averagePrice":0,"priceSpread":"X%","l1Vendor":""},"technoCommercialRanking":[{"rank":1,"vendor":"","score":0,"rationale":""}],"recommendation":{"bestValue":"vendor name","rationale":"why","conditions":"any conditions for this recommendation"},"negotiationStrategy":["strategy point"]}`,

"negotiation-brief":(f)=>`You are a world-class procurement negotiation strategist with deep expertise in Indian manufacturing supplier negotiations.

Generate a comprehensive negotiation strategy brief for this upcoming negotiation:

SUPPLIER: ${f.supplierName||"Not specified"}
ITEMS: ${f.items||"Not specified"}
ANNUAL SPEND: ₹${f.currentSpend||0} Lakhs
CONTRACT STATUS: ${f.contractType||"Not specified"}
PRIMARY OBJECTIVE: ${f.objective||"Cost reduction"}
KNOWN LEVERAGE: ${f.leverage||"None specified"}
CONSTRAINTS: ${f.constraints||"None specified"}
RELATIONSHIP HISTORY: ${f.history||"Not specified"}

RESPOND ONLY VALID JSON:
{"briefTitle":"","supplierAssessment":{"profile":"assessment of supplier's likely position","dependency":"how dependent are they on this business","alternatives":"what alternatives does buyer have","marketPosition":"supplier's market standing"},"leverageAnalysis":[{"point":"leverage point","strength":"Strong|Medium|Weak","howToUse":"specific tactic"}],"strategy":{"approach":"Collaborative|Competitive|Balanced","openingPosition":"what to open with","targetOutcome":"realistic best case","walkAway":"minimum acceptable","concessionPlan":[{"give":"what you can concede","get":"what you get in return","sequence":"when to play this"}]},"talkingPoints":[{"point":"data-backed argument","data":"supporting evidence","expectedResponse":"how supplier might counter","rebuttal":"your counter to their counter"}],"redLines":["things NOT to concede"],"timeline":{"prepActions":["action before meeting"],"duringMeeting":["agenda flow"],"followUp":["post-meeting actions"]},"riskMitigation":"what if negotiation fails — Plan B"}`
};

// ─── MODULE CONFIGS (compact) ───
const RPT_SNIP={
  "spend":{
    summary:["Steel accounts for 58.3% of total spend (₹494 Cr) with procurement concentrated across just 4 suppliers — a 2% reduction yields ₹9.8 Cr savings","Tail spend of ₹38.6 Cr spread across 287 suppliers (70% of supplier base contributing only 4.6% of spend)","Maverick spend at 8.0% (₹67.4 Cr) — 2,847 POs placed without rate contracts"],
    metrics:[{m:"Total Spend",v:"₹847 Cr",t:"up"},{m:"Active Suppliers",v:"412",t:"stable"},{m:"Maverick Spend",v:"8.0%",t:"up"},{m:"Savings Identified",v:"₹68.2 Cr",t:"down"}],
    findings:[{f:"Steel price not linked to market index — missing ₹800-1,200/MT downward corrections",i:"₹9.8 Cr/yr",a:"Migrate to monthly HRC index-linked pricing"},{f:"Inter-plant price variance of 4-18% for identical items across Ludhiana, Murthal and WB",i:"₹8.4 Cr/yr",a:"Centralize rate contracts — best-plant-price policy"},{f:"MRO spares purchased at MRP without rate contracts from 94 suppliers",i:"₹4.0 Cr/yr",a:"E-catalogue + reverse auction for top 200 items"}],
    risks:[{r:"Single-source dependency for 23 items worth ₹112 Cr",s:"High"},{r:"No commodity hedging for ₹494 Cr steel exposure",s:"Medium"}]},
  "price-variance":{
    summary:["Average unit price increase of 3.8% across all materials — above WPI inflation of 2.9%","Top 5 items by price impact account for ₹14.2 Cr additional cost YoY","3 suppliers increased prices 12-18% without corresponding commodity movement"],
    metrics:[{m:"Items Analyzed",v:"2,847",t:"stable"},{m:"Price Increases",v:"42%",t:"up"},{m:"Price Decreases",v:"28%",t:"down"},{m:"Unjustified Increases",v:"₹8.6 Cr",t:"up"}],
    findings:[{f:"HR Coil price up 6.2% while steel index moved only 3.1% — excess ₹1,540/MT",i:"₹4.8 Cr/yr",a:"Renegotiate with Tata/JSW using index data"},{f:"Polymer components up 11% despite PP prices dropping 8% in same period",i:"₹2.4 Cr/yr",a:"Immediate price revision with commodity evidence"},{f:"Rim prices unchanged at ₹142 for 18 months — should have dropped with aluminum",i:"₹1.8 Cr/yr",a:"Share LME aluminum chart in supplier meeting"}],
    risks:[{r:"8 suppliers gave >10% increase — 3 without justification",s:"High"},{r:"Commodity-linked pricing covers only 24% of addressable spend",s:"Medium"}]},
  "inventory-health":{
    summary:["Total inventory value ₹142 Cr — inventory turnover ratio of 6.4 against target of 12","Non-moving stock (NMS >12 months) at ₹18.7 Cr — 13.2% of total inventory value","Excess stock of ₹34 Cr identified against 3-month rolling consumption"],
    metrics:[{m:"Total Inventory",v:"₹142 Cr",t:"up"},{m:"ITR",v:"6.4x",t:"up"},{m:"NMS Value",v:"₹18.7 Cr",t:"up"},{m:"Excess Stock",v:"₹34 Cr",t:"up"}],
    findings:[{f:"Government tender-specific items (₹8.2 Cr) lying unsold — tender lost/cancelled",i:"₹8.2 Cr WC",a:"Liquidate at 30% recovery or rework for commercial models"},{f:"Imported e-bike components ordered in bulk (MOQ driven) — 8 months stock on hand",i:"₹12.4 Cr WC",a:"Negotiate smaller MOQ with Bafang/Samsung or consignment model"},{f:"Safety stock levels set at 45 days for all items — no ABC differentiation",i:"₹22 Cr WC",a:"A items: 15 days, B: 25 days, C: 45 days — releases ₹22 Cr"}],
    risks:[{r:"NMS provision of ₹18.7 Cr will hit P&L if not liquidated by year-end",s:"High"},{r:"Obsolescence risk on e-bike battery cells — technology changing rapidly",s:"Medium"}]},
  "supplier-scorecard":{
    summary:["Only 34% of suppliers rated Green (OTIF >95%, PPM <500) — 18% are in Red zone","Top 3 suppliers by spend have declining quality trend — PPM up 40% over 6 months","Average delivery OTIF at 78% against 95% target — causing ₹2.1 Cr overtime costs"],
    metrics:[{m:"Suppliers Rated",v:"86",t:"stable"},{m:"Green Zone",v:"34%",t:"down"},{m:"Avg OTIF",v:"78%",t:"up"},{m:"Avg PPM",v:"1,840",t:"up"}],
    findings:[{f:"Rim supplier OTIF at 62% — 38% of deliveries late by 5+ days, causing line stoppages",i:"₹1.8 Cr/yr",a:"Issue SCN, start qualification of Superking as alternate"},{f:"Chain supplier PPM at 3,200 — rejections causing rework at ₹4.2/piece on 6L units",i:"₹25.2L/yr",a:"Supplier development program + incoming inspection tightening"},{f:"15 suppliers have zero quality data — no incoming inspection being done",i:"Risk",a:"Implement mandatory incoming QC for all A-category items"}],
    risks:[{r:"3 critical suppliers trending Red — need immediate corrective action",s:"High"},{r:"No supplier audit program — last audits done 2+ years ago for 70% of base",s:"Medium"}]},
  "category-opportunity":{
    summary:["₹148 Cr total savings opportunity identified across 18 categories — 8.4% of addressable spend","Steel (₹494 Cr) positioned as Leverage — maximum negotiation power but underutilized","5 categories have zero competitive tension — single source with no recent benchmarking"],
    metrics:[{m:"Categories Analyzed",v:"18",t:"stable"},{m:"Total Opportunity",v:"₹148 Cr",t:"down"},{m:"Quick Wins (<90 days)",v:"₹42 Cr",t:"down"},{m:"Single-Source Categories",v:"5",t:"up"}],
    findings:[{f:"Steel: Index-linked pricing + volume consolidation across plants",i:"₹16 Cr/yr",a:"Lever: Negotiate | Timeline: 60 days"},{f:"Packaging: Commodity-linked repricing + supplier consolidation",i:"₹3.4 Cr/yr",a:"Lever: Market benchmark | Timeline: 30 days"},{f:"MRO: E-catalogue + demand aggregation + reverse auction",i:"₹6.5 Cr/yr",a:"Lever: Process improvement | Timeline: 90 days"}],
    risks:[{r:"Battery cells in Bottleneck quadrant — limited supply options, high switching cost",s:"High"},{r:"5 categories have no strategy document — ad-hoc buying at spot rates",s:"Medium"}]},
  "cost-reduction-tracker":{
    summary:["Pipeline has ₹96 Cr identified savings but only ₹28.4 Cr (29.6%) implemented YTD","34 initiatives stuck in pipeline >90 days — ownership gaps and approval delays","Procurement lever contributes 62% of savings; VaVe contributes only 8% (untapped)"],
    metrics:[{m:"Target Savings",v:"₹40 Cr",t:"stable"},{m:"Achieved YTD",v:"₹28.4 Cr",t:"down"},{m:"Pipeline Value",v:"₹96 Cr",t:"down"},{m:"Conversion Rate",v:"29.6%",t:"up"}],
    findings:[{f:"34 initiatives blocked >90 days — mostly pending design/quality approval",i:"₹18 Cr stuck",a:"Escalation matrix + weekly review with cross-functional team"},{f:"VaVe projects contribute only 8% — specification changes barely explored",i:"₹12 Cr untapped",a:"Launch VaVe workshops for top 20 high-cost items"},{f:"Savings double-counted on 6 items — same reduction claimed by 2 buyers",i:"₹3.2 Cr overcount",a:"Implement unique initiative ID system with finance validation"}],
    risks:[{r:"₹11.6 Cr gap to reach ₹40 Cr target with only 2 months remaining",s:"High"},{r:"Cost avoidance being mixed with hard savings — inflating reported numbers",s:"Medium"}]},
  "savings-validation":{
    summary:["Of ₹42 Cr claimed savings, only ₹29.8 Cr (71%) validated by P&L cross-check","₹6.2 Cr identified as cost avoidance misclassified as hard savings","3 initiatives (₹4.1 Cr) use budget as baseline instead of last transacted price — methodology failure"],
    metrics:[{m:"Claimed Savings",v:"₹42 Cr",t:"stable"},{m:"Validated (Hard)",v:"₹29.8 Cr",t:"down"},{m:"Validation Rate",v:"71%",t:"down"},{m:"Methodology Issues",v:"8 items",t:"up"}],
    findings:[{f:"₹4.1 Cr uses budget baseline — actual last price was already lower than budget",i:"₹4.1 Cr overstatement",a:"Mandate last-transacted-price as baseline for all savings"},{f:"₹6.2 Cr cost avoidance (prevented increase) mixed with reduction — not P&L visible",i:"₹6.2 Cr reclassify",a:"Separate reporting: hard savings vs avoidance vs one-time"},{f:"Steel savings of ₹8.4 Cr correctly validated — formula pricing delivering as modeled",i:"₹8.4 Cr confirmed",a:"Continue index-linked approach; extend to more suppliers"}],
    risks:[{r:"CFO credibility risk if unvalidated numbers reach board",s:"High"},{r:"No standardized savings methodology document — each buyer calculates differently",s:"Medium"}]},
  "supplier-risk":{
    summary:["23 items worth ₹112 Cr depend on single source — any disruption stops production in 5-18 days","Geographic concentration: 64% of spend within 200km of Ludhiana — one regional event impacts majority","Top 5 suppliers cover 55% of total spend — loss of any one creates 3-6 week recovery gap"],
    metrics:[{m:"Single-Source Items",v:"23",t:"up"},{m:"Spend at Risk",v:"₹112 Cr",t:"up"},{m:"Avg Buffer Days",v:"12",t:"up"},{m:"Suppliers Assessed",v:"86",t:"stable"}],
    findings:[{f:"Rims: 78% from Avon (competitor) — no qualified alternate for alloy rims",i:"₹34 Cr at risk",a:"Qualify Superking + Enkei India within 120 days"},{f:"Battery cells: 100% import, single source, 12-week lead time",i:"₹22 Cr at risk",a:"Qualify BYD/CATL; maintain 8-week safety stock meanwhile"},{f:"Chain supplier located in flood-prone zone — disrupted twice in last 3 years",i:"₹28 Cr at risk",a:"Qualify alternate in Gujarat/Rajasthan; sign VMI agreement"}],
    risks:[{r:"Production loss estimated at ₹2.8 Cr/day if top steel supplier disrupted",s:"High"},{r:"No formal BCP (Business Continuity Plan) with any supplier",s:"High"}]}
};
const MCFG={
"transport":{
  aiHint:"Upload a transporter quotation or freight bill to auto-extract cost items",
  help:{when:"Use when you need to calculate the per-piece freight cost for any route — for landed cost analysis, logistics optimization, or comparing transporters. Essential before supplier price negotiations that include 'FOR destination' pricing.",howTo:["Enter origin and destination cities, distance in km","Select vehicle type (affects payload capacity)","Enter total load weight (kg) and number of pieces per load","Add cost items: diesel, tolls, driver, loading/unloading, insurance, overhead","Use templates for common routes or build from scratch"],output:["Total freight cost for the load","₹/kg freight rate","₹/piece freight cost — use this in your landed cost or TCO calculations"],tips:["Get actual toll costs from NHAI toll calculator","Diesel rate changes monthly — use current rate from IOC/BPCL","For part-load (LTL), use per-kg market rates instead of FTL calculation","Include return empty leg cost if vehicle doesn't get return load"]},
  fields:[{k:"origin",l:"Origin",ph:"Pune, Maharashtra"},{k:"destination",l:"Destination",ph:"Chennai, TN"},{k:"distanceKm",l:"Distance (km)",t:"number"},{k:"vehicleType",l:"Vehicle",t:"select",opts:["Tata Ace (1T)","Tata 407 (2.5T)","Eicher 1110 (6T)","Tata 1109 (9T)","16T Truck","20ft Container","40ft Trailer"]},{k:"totalWeightKg",l:"Total Weight (kg)",t:"number"},{k:"piecesPerLoad",l:"Pieces/Load",t:"number"}],
  cols:[{key:"item",label:"Cost Item",w:150},{key:"description",label:"Basis",w:200,ph:"..."},{key:"amount",label:"₹",type:"number",w:90,step:"100",def:0},{key:"category",label:"Type",type:"select",w:80,options:["fuel","toll","driver","loading","insurance","overhead","other"],def:"fuel"}],
  addLabel:"Add Cost Item",sectionTitle:"💰 Freight Cost Breakdown",
  calcFn:(ops,cfg)=>{let t=0;ops.forEach(o=>t+=Number(o.amount)||0);return{total:t,perKg:cfg.totalWeightKg>0?t/cfg.totalWeightKg:0,perPiece:cfg.piecesPerLoad>0?t/cfg.piecesPerLoad:0}},
  barFn:(r,c)=>[{l:"Total",v:"₹"+r.total.toFixed(0)},{l:"Per Kg",v:"₹"+r.perKg.toFixed(2)},{l:"Per Piece",v:"₹"+r.perPiece.toFixed(2),hl:1}],
  resultLabel:"Total Freight",
  templates:{
    t1:{label:"🚛 Pune→Chennai (1400km)",cfg:{origin:"Pune, MH",destination:"Chennai, TN",distanceKm:1400,vehicleType:"16T Truck",totalWeightKg:14000,piecesPerLoad:6000},ops:[{item:"Diesel",description:"1400km ÷ 5km/l × ₹90/l",amount:25200,category:"fuel"},{item:"Tolls",description:"NH48/NH44",amount:3800,category:"toll"},{item:"Driver",description:"2 days × ₹1200",amount:2400,category:"driver"},{item:"Loading+Unloading",description:"Both ends",amount:4000,category:"loading"},{item:"Insurance",description:"0.1%",amount:1400,category:"insurance"},{item:"OH+Profit",description:"15%",amount:5520,category:"overhead"}]},
    blank:{label:"📝 Custom",cfg:{origin:"",destination:"",distanceKm:0,vehicleType:"16T Truck",totalWeightKg:0,piecesPerLoad:0},ops:[]},
  },
  prompt:`Logistics cost analyst for Indian road transport. Extract cost items from the document. Generate JSON with costSheet array. Each item must have: item (cost name), description (calculation basis), amount (₹ value), category (one of: fuel, toll, driver, loading, insurance, overhead, other). RESPOND ONLY VALID JSON: {"costSheet":[{"item":"str","description":"str","amount":0,"category":"fuel"}]}`,
},
"packaging":{
  aiHint:"Upload a packaging BOM or supplier quotation to auto-extract items",
  help:{when:"Use when you need the exact packaging cost per piece — for total landed cost, supplier price comparison (especially when one supplies packaged and another doesn't), or packaging optimization projects.",howTo:["Enter pieces per box and boxes per pallet (for reference)","Add each packaging item: material name, quantity used per piece, and unit cost","For shared items (e.g., 1 box holds 4 pieces), set qty as fraction (0.25 = ¼ box per piece)","Include packing labor — enter minutes per piece and ₹/minute rate","Categorize as primary (VCI paper, foam), secondary (box), or tertiary (pallet)"],output:["Total packaging cost per piece","Breakdown by primary, secondary, tertiary, and labor","Use this in your total landed cost or TCO model"],tips:["Most companies underestimate packaging cost by 30-50%","VCI paper is mandatory for export — don't skip it","Returnable packaging can save 40-60% over 12 months for regular routes","Get box rates from corrugator directly, not through trader"]},
  fields:[{k:"piecesPerBox",l:"Pieces/Box",t:"number"},{k:"boxesPerPallet",l:"Boxes/Pallet",t:"number"}],
  cols:[{key:"name",label:"Item",w:160},{key:"qty",label:"Qty/Pc",type:"number",w:80,step:"0.01",def:1},{key:"unitCost",label:"₹/Unit",type:"number",w:90,step:"0.5",def:5},{key:"_cost",label:"Cost/Pc",w:80,computed:r=>(Number(r.qty)||0)*(Number(r.unitCost)||0)},{key:"category",label:"Type",type:"select",w:80,options:["primary","secondary","tertiary","labor"],def:"primary"}],
  addLabel:"Add Item",sectionTitle:"📦 Packaging Items",
  calcFn:(ops)=>{let t=0;ops.forEach(i=>t+=(Number(i.qty)||0)*(Number(i.unitCost)||0));return{total:t}},
  barFn:r=>[{l:"Cost/Piece",v:"₹"+r.total.toFixed(2),hl:1}],resultLabel:"Packaging ₹/Piece",
  templates:{
    t1:{label:"⚙️ Motor Housing",cfg:{piecesPerBox:4,boxesPerPallet:12},ops:[{name:"VCI Paper",qty:1,unitCost:3.5,category:"primary"},{name:"Foam Insert",qty:1,unitCost:4,category:"primary"},{name:"Corrugated Box 5-ply",qty:0.25,unitCost:35,category:"secondary"},{name:"Pallet(amortized)",qty:0.005,unitCost:350,category:"tertiary"},{name:"Packing Labor(min)",qty:0.5,unitCost:4,category:"labor"}]},
    blank:{label:"📝 Custom",cfg:{piecesPerBox:1,boxesPerPallet:1},ops:[]},
  },
  prompt:`Packaging engineer. Extract packaging items from document. Generate JSON with costSheet array. Each item must have: name (item name), qty (quantity per piece, use fractions like 0.25 if shared), unitCost (₹ per unit), category (one of: primary, secondary, tertiary, labor). RESPOND ONLY VALID JSON: {"costSheet":[{"name":"str","qty":1,"unitCost":5,"category":"primary"}]}`,
},
"landed":{
...
const toArrBuf=f=>new Promise((r,j)=>{const x=new FileReader();x.onload=()=>r(x.result);x.onerror=()=>j("fail");x.readAsArrayBuffer(f)});
const toText=f=>new Promise((r,j)=>{const x=new FileReader();x.onload=()=>r(x.result);x.onerror=()=>j("fail");x.readAsText(f)});
...
async function callAI(files,prompt,notes){
  ...
  for(const f of files){
    ...
    if(["xlsx","xls","csv","tsv"].includes(ext)){
      // Parse spreadsheet and send as text
      try{
        const data=await parseSpreadsheet(f);
        const preview=data.length>80000?data.slice(0,80000)+"\n...[truncated at 80K chars]":data;
        content.push({type:"text",text:`[DATA FROM FILE: ${f.name}]\n${preview}`});
      }catch(pe){content.push({type:"text",text:`[File: ${f.name} — could not parse: ${pe.message}]`})}
    }else{
      const b=await toB64(f),mt=mtype(f);
      content.push(mt.startsWith("image/")?{type:"image",source:{type:"base64",media_type:mt,data:b}}:{type:"document",source:{type:"base64",media_type:mt,data:b}});
    }
  }
  content.push({type:"text",text:prompt+"\nNotes: "+(notes||"None")});
  let r=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-5-20250929",max_tokens:8192,temperature:0,messages:[{role:"user",content}]})});
  if(!r.ok){const errBody=await r.text().catch(()=>"");throw new Error("API "+r.status+(errBody?": "+errBody.slice(0,200):""))}
  const d=await r.json(),txt=d.content?.map(c=>c.text||"").join("")||"";
  const i1=txt.indexOf("{"),i2=txt.lastIndexOf("}");if(i1===-1||i2<=i1)throw new Error("No JSON in response");return JSON.parse(txt.slice(i1,i2+1));
}
...
function FileUpload({files,onChange}){const ref=useRef(null);return(<div className="fu" onClick={()=>ref.current?.click()}>
  ...
  {files.length>0&&<div className="fu-chips" onClick={e=>e.stopPropagation()}>{files.map((f,i)=><span key={i} className="fu-chip">{f.name}<button onClick={()=>onChange(files.filter((_,j)=>j!==i))}>✕</button></span>)}</div>}</div>);
}
...
function EbookCover({book,size="md"}){
  ...
  const toc=EBOOK_TOC[book.id]||[];
  if(showToc)return(
    <div style={{width:w,height:h,background:book.bg,borderRadius:8,overflow:"hidden",position:"relative",boxShadow:"4px 4px 20px rgba(0,0,0,.4)",cursor:"pointer",flexShrink:0,border:"1px solid "+book.accent+"44"}} onClick={()=>setShowToc(false)}>
      <div style={{padding:size==="sm"?"6px":"10px 12px",height:"100%",display:"flex",flexDirection:"column",overflow:"hidden"}}>
        <div style={{fontSize:7,fontWeight:700,color:book.accent,letterSpacing:1,textTransform:"uppercase",marginBottom:2}}>Contents</div>
        <div style={{fontSize:size==="sm"?8:9,fontWeight:700,color:"#fff",marginBottom:6,lineHeight:1.2,fontFamily:"'Bricolage Grotesque'"}}>{book.title}</div>
        <div style={{flex:1,overflow:"hidden"}}>{toc.map((ch,i)=><div key={i} style={{display:"flex",gap:4,alignItems:"flex-start",padding:(size==="sm"?"1":"2")+"px 0",borderBottom:"1px solid rgba(255,255,255,.06)"}}>
          <span style={{fontSize:7,fontFamily:"IBM Plex Mono",color:book.accent,fontWeight:700,flexShrink:0,width:12,textAlign:"right"}}>{String(i+1)+"."}</span>
          <span style={{fontSize:size==="sm"?6.5:7.5,color:"rgba(255,255,255,.8)",lineHeight:1.3}}>{ch}</span>
        </div>)}</div>
        <div style={{fontSize:6,color:"rgba(255,255,255,.4)",textAlign:"center",marginTop:4}}>{book.pages} pages</div>
      </div>
    </div>
  );
  return(
    <div style={{width:w,height:h,background:book.bg,borderRadius:8,overflow:"hidden",position:"relative",boxShadow:"4px 4px 20px rgba(0,0,0,.4),inset 0 1px 0 rgba(255,255,255,.05)",cursor:"pointer",flexShrink:0,border:"1px solid rgba(255,255,255,.06)"}} onClick={()=>setShowToc(true)}>
      ...
      <div style={{position:"absolute",bottom:0,left:0,right:0,padding:size==="sm"?"8px":"12px"}}>
        ...
        <div style={{fontSize:7,color:"rgba(255,255,255,.5)"}}>Procurement Experts</div>
      </div>
    </div>
  );
}
...
const SC_HELP={when:"Use when you need to build a bottom-up zero-based cost for a manufactured component — before supplier negotiations, during new product development, or to validate an existing supplier's price. Ideal for machined, stamped, fabricated, or assembled parts.",howTo:["Upload an engineering drawing (PDF/image) OR load a template and edit manually","Fill Raw Materials table: select material, grade, net weight (kg), and ₹/kg rate. Weight is auto-multiplied by 1.08 for scrap/wastage","Build the Process Sheet: add each manufacturing operation, machine used, cycle time (minutes), and ₹/minute machine rate","Add Secondary Operations: surface treatments like plating, painting, powder coating — enter ₹/piece","Overhead (15%) and Profit (8%) are auto-added to the subtotal"],output:["Raw Material cost per piece (material × weight × 1.08 scrap)","Process cost per piece (sum of cycle time × machine rate for each operation)","Secondary operations cost","Overhead and profit breakdown","Final should-cost per piece for negotiation"],tips:["Get cycle times from your production engineering team or time-study data","Machine rates should include depreciation, power, operator, consumables","Compare your should-cost vs. supplier quote to identify negotiation leverage","Use 1.08x multiplier for sheet metal (8% scrap), adjust for casting (1.15-1.20)"]};
...
function GenericModule({moduleId,onResult,credits,useCredit}){
  ...
  const doAI=async()=>{if(!files.length||credits<1)return;setLoading(true);setErr("");try{useCredit();const ext=await callAI(files,cfg0.prompt,notes+"\n"+JSON.stringify(fields));const ok=Object.keys(ext).find(k=>Array.isArray(ext[k])&&ext[k].length>0);if(ok){setOps(ext[ok]);setShowAI(false)}}catch(e){setErr("AI: "+e.message)}setLoading(false)};
  ...
  return(<div className="mod-edit">
    ...
    <div className="ai-assist-zone">
      ...
      {showAI&&<div className="ai-panel">
        ...
        {credits<1&&<div className="upsell-inline" style={{marginTop:6}}>🔒 AI extraction requires Pro plan. <span className="upsell-link" onClick={()=>window.__costlens_upgrade?.()}>Upgrade → ₹1,999/mo</span></div>}
      </div>}
    </div>
    {/* Calculate */}
...
function ShouldCostModule({onResult,credits,useCredit}){
  ...
  const doAI=async()=>{if(!files.length||credits<1)return;setLoading(true);setErr("");try{useCredit();const e=await callAI(files,SC_PROMPT,notes);setExt(e);if(e.processSheet)setOps(e.processSheet);if(e.secondaryOps)setSec(e.secondaryOps);if(e.components){const mg=new Map();e.components.forEach(c=>{const cat=c.materialCategory||"Mild Steel / CRS";if(mg.has(cat))mg.get(cat).wt+=((Number(c.weightKg)||0)*(Number(c.qty)||1));else mg.set(cat,{mat:cat,grade:c.grade||"",wt:(Number(c.weightKg)||0)*(Number(c.qty)||1),rate:String(matR(cat))})});setRm([...mg.values()].map(g=>({...g,wt:String(g.wt.toFixed(3))})))}}catch(e){setErr("AI: "+e.message)}setLoading(false);setStep("edit")};
  const calc=()=>{
    // RM with details
    ...
    // OH + Profit
    const oh=(pT+sT)*OH,sub=rmT+pT+sT+oh,pr=sub*PROFIT,tot=sub+pr;
    // Detailed breakdown
    const bd={rawMaterial:{amount:rmT,details:rmD.join("; ")||"—"},processing:{amount:pT,details:`${ops.length} operations, ${totalCycle.toFixed(1)} min total`},secondary:{amount:sT,details:sD.length?sD.join("; "):"None"},overheads:{amount:oh,details:`15% × ₹${(pT+sT).toFixed(2)} = ₹${oh.toFixed(2)}`},profit:{amount:pr,details:`8% × ₹${sub.toFixed(2)} = ₹${pr.toFixed(2)}`}};
    // Volume slabs
    const q=100000;const slabs={base:{volume:q,costPerPiece:tot}};
    ["plus20","plus30","plus40"].forEach((sk,si)=>{const m=[1.2,1.3,1.4][si];const v=Math.round(q*m);const d=VOL_D[sk];const vRm=rmT*d.rm,vProc=pT*d.proc,vSec=sT*d.sec;const vOh=(vProc+vSec)*OH;const vSub=vRm+vProc+vSec+vOh;const vTot=vSub+vSub*PROFIT;slabs[sk]={volume:v,costPerPiece:vTot,savingsPct:((tot-vTot)/tot)*100,rmDiscount:((1-d.rm)*100).toFixed(1)}});
    // Weight + confidence
    ...
    onResult({module:"should-cost",name:ext?.partName||notes||"Component",date:new Date().toLocaleString("en-IN"),total:tot,
      ...
      costBreakdown:bd,volumeSlabs:slabs,negotiationPoints:ext?.negotiationPoints||[],
      ops:[...ops],sec:[...sec],rm:[...rm],
      details:{rm:rmT,proc:pT,sec:sT,oh,profit:pr},resultVal:"₹"+tot.toFixed(2)})};
  ...
  const pT=ops.reduce((s,o)=>s+(Number(o.cycleTimeMin)||0)*(Number(o.ratePerMin)||0),0),rmT=rm.reduce((s,r)=>s+(Number(r.wt)||0)*(Number(r.rate)||0)*1.08,0),sT2=sec.reduce((s,x)=>s+(Number(x.costPerPiece)||0),0),oh2=(pT+sT2)*OH,sub2=rmT+pT+sT2+oh2,tot2=sub2+sub2*PROFIT;
  if(step==="input")return(<div className="mod-in"><HelpGuide help={SC_HELP}/><FileUpload files={files} onChange={setFiles}/><textarea className="mod-notes" value={notes} onChange={e=>setNotes(e.target.value)} placeholder="Part name, specs..."/>
    {loading&&<div className="mod-loading">🤖 Building process sheet...</div>}{err&&<div className="mod-err">{err}</div>}
    <div className="tpl-zone"><div className="tpl-title">Load Template</div><div className="tpl-btns">{Object.entries(SC_T).map(([k,t])=><button key={k} className="tpl-btn" onClick={()=>loadTpl(k)}>{t.label}</button>)}</div></div>
    <div className="mod-actions"><button className="btn-sec" onClick={()=>setStep("edit")}>Manual →</button>{files.length>0&&credits>=1&&<button className="btn-pri" onClick={doAI} disabled={loading}>✨ AI Extract →</button>}{files.length>0&&credits<1&&<button className="upsell-btn" onClick={()=>window.__costlens_upgrade?.()}>🔒 Upgrade for AI</button>}</div>
    {credits<1&&<div className="upsell-inline">💡 <strong>Pro tip:</strong> Upload your drawing → AI builds the complete process sheet, BOM, and should-cost in 30 seconds. <span className="upsell-link" onClick={()=>window.__costlens_upgrade?.()}>Upgrade to Pro →</span></div>}
    </div>);
  return(<div className="mod-edit">
    {!ext&&credits>0&&<div className="upsell-bar ai-nudge" onClick={()=>setStep("input")}>✨ <strong>Filling manually?</strong> Upload a drawing and AI builds this process sheet in 30 seconds → <span className="upsell-link">Try AI Extract</span></div>}
    {!ext&&credits<1&&<div className="upsell-bar upgrade-nudge" onClick={()=>window.__costlens_upgrade?.()}>🚀 <strong>Upgrade to Pro</strong> — AI reads your engineering drawing and generates the complete process sheet, material list, and cost estimate automatically → <span className="upsell-link">Upgrade ₹1,999/mo</span></div>}
    <div className="mod-bar"><div className="mod-stat"><span className="ms-l">RM</span><span className="ms-v">₹{rmT.toFixed(0)}</span></div><div className="mod-stat"><span className="ms-l">Process</span><span className="ms-v">₹{pT.toFixed(0)}</span></div><div className="mod-stat hl"><span className="ms-l">Est.Total</span><span className="ms-v">₹{tot2.toFixed(2)}</span></div></div>
    ...
    <div className="section-head">🎨 Secondary</div><OpsTable cols={secC} data={sec} onChange={setSec} addLabel="Add Finishing"/>
    <div className="section-head">💰 Raw Materials</div><OpsTable cols={rmC} data={rm} onChange={setRm} addLabel="Add Material"/>
    <div className="mod-actions"><button className="btn-sec" onClick={()=>setStep("input")}>← Back</button><button className="btn-pri" onClick={calc} disabled={ops.length===0}>⚡ Calculate</button></div></div>);
}
function ToolCostModule({onResult,credits,useCredit}){
  ...
  const doAI=async()=>{if(!files.length||credits<1)return;setLoading(true);setErr("");try{useCredit();const ext=await callAI(files,TC_PROMPT,notes);if(ext.operationSheet)setOps(ext.operationSheet);if(ext.secondaryCosts)setSec(ext.secondaryCosts)}catch(e){setErr("AI: "+e.message)}setLoading(false);setStep("edit")};
  const calc=()=>{let matT=0;rm.forEach(r=>matT+=(Number(r.wt)||0)*(Number(r.rate)||0));let machT=0;ops.forEach(o=>machT+=(Number(o.hoursEstimate)||0)*(Number(o.ratePerHour)||0));let buyT=0;sec.forEach(s=>buyT+=Number(s.cost)||0);const total=matT+machT+buyT,life=Number(tl)||100000;
    onResult({module:"tool-cost",name:notes||"Tool/Die",date:new Date().toLocaleString("en-IN"),total,details:{material:matT,machining:machT,boughtOut:buyT,toolLife:life,amortizedPerPiece:total/life},ops:[...ops],resultVal:"₹"+total.toFixed(0)})};
  ...
  let matT=0;rm.forEach(r=>matT+=(Number(r.wt)||0)*(Number(r.rate)||0));let machT=0;ops.forEach(o=>machT+=(Number(o.hoursEstimate)||0)*(Number(o.ratePerHour)||0));let buyT=0;sec.forEach(s=>buyT+=Number(s.cost)||0);const total=matT+machT+buyT;
  if(step==="input")return(<div className="mod-in"><HelpGuide help={TC_HELP}/><FileUpload files={files} onChange={setFiles}/><textarea className="mod-notes" value={notes} onChange={e=>setNotes(e.target.value)} placeholder="Tool type, material..."/>
    <div className="mod-field"><label>Tool Life (shots)</label><input type="number" value={tl} onChange={e=>setTl(e.target.value)} style={{maxWidth:200}}/></div>
    {loading&&<div className="mod-loading">🤖 Analyzing...</div>}{err&&<div className="mod-err">{err}</div>}
    <div className="tpl-zone"><div className="tpl-title">Load Template</div><div className="tpl-btns">{Object.entries(TC_T).map(([k,t])=><button key={k} className="tpl-btn" onClick={()=>loadTpl(k)}>{t.label}</button>)}</div></div>
    <div className="mod-actions"><button className="btn-sec" onClick={()=>setStep("edit")}>Manual →</button>{files.length>0&&credits>=1&&<button className="btn-pri" onClick={doAI} disabled={loading}>✨ AI Extract →</button>}{files.length>0&&credits<1&&<button className="upsell-btn" onClick={()=>window.__costlens_upgrade?.()}>🔒 Upgrade for AI</button>}</div>
    {credits<1&&<div className="upsell-inline">💡 <strong>Pro tip:</strong> Upload a tool drawing → AI extracts operations, steel weight, and bought-out items automatically. <span className="upsell-link" onClick={()=>window.__costlens_upgrade?.()}>Upgrade to Pro →</span></div>}
    </div>);
  return(<div className="mod-edit">
    <div className="mod-bar"><div className="mod-stat"><span className="ms-l">Material</span><span className="ms-v">₹{matT.toFixed(0)}</span></div><div className="mod-stat"><span className="ms-l">Machining</span><span className="ms-v">₹{machT.toFixed(0)}</span></div><div className="mod-stat hl"><span className="ms-l">Total</span><span className="ms-v">₹{total.toFixed(0)}</span></div><div className="mod-stat" style={{color:"#10b981"}}><span className="ms-l">Amort/pc</span><span className="ms-v">₹{(total/(Number(tl)||100000)).toFixed(2)}</span></div></div>
    <div className="section-head">🔧 Operations</div><OpsTable cols={opC} data={ops} onChange={setOps} addLabel="Add Op"/>
    <div className="section-head">🛒 Bought-out</div><OpsTable cols={secC} data={sec} onChange={setSec} addLabel="Add Item"/>
    <div className="section-head">🪨 Tool Steel</div><OpsTable cols={rmC} data={rm} onChange={setRm} addLabel="Add Material"/>
    <div className="mod-field" style={{marginTop:6}}><label>Tool Life (shots)</label><input type="number" value={tl} onChange={e=>setTl(e.target.value)} style={{maxWidth:200}}/></div>
    <div className="mod-actions"><button className="btn-sec" onClick={()=>setStep("input")}>← Back</button><button className="btn-pri" onClick={calc} disabled={ops.length===0}>⚡ Calculate</button></div></div>);
}
  ...
  if(isSC&&result.costBreakdown){
    ...
    return(<div className="result-view">
      ...
      <div style={{display:"flex",gap:4,marginBottom:14,flexWrap:"wrap"}}>
        <span className="rv-badge">⚖️ {String(result.estimatedWeight||"")}</span>
        ...
        <span className={`rv-badge ${result.confidence==="HIGH"?"rv-badge-ok":result.confidence==="LOW"?"rv-badge-warn":"rv-badge-med"}`}>{String(result.confidence||"")}</span>
        <span className="rv-badge rv-badge-ok">Zero-Based</span>
      </div>
      {/* Volume Slabs */}
      {vs&&<div style={{position:"relative"}}><div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:1,background:"var(--s2)",border:"1px solid var(--s2)",borderRadius:8,overflow:"hidden",marginBottom:14}}>
        {VOL_SLABS.map(sk=>{const s=vs[sk.key];if(!s)return null;const locked=plan==="free"&&sk.key!=="base";return(<div key={sk.key} style={{background:"var(--s1)",padding:"8px 10px",textAlign:"center",filter:locked?"blur(4px)":"none",userSelect:locked?"none":"auto"}}>
      ...
      </div>{plan==="free"&&<div className="upsell-volume-overlay" onClick={()=>window.__costlens_upgrade?.()}>🔒 Volume discounts available with Pro <span className="upsell-link">Upgrade →</span></div>}</div>}
      ...
      <table className="rv-table"><thead><tr><th style={{textAlign:"left"}}>Element</th><th style={{textAlign:"right"}}>₹</th><th style={{textAlign:"right"}}>%</th><th style={{textAlign:"left"}}>Details</th></tr></thead>
        <tbody>{SC_CATS.map(cat=>{const a=bd[cat.key]?.amount||0;if(cat.key==="secondary"&&a===0)return null;return(<tr key={cat.key}><td><span style={{display:"inline-block",width:7,height:7,borderRadius:"50%",background:cat.color,marginRight:6,verticalAlign:"middle"}}/>{cat.label}</td><td style={{textAlign:"right",fontFamily:"'IBM Plex Mono'",fontWeight:600}}>₹{a.toFixed(2)}</td><td style={{textAlign:"right",color:"var(--t3)",fontSize:10}}>{t>0?((a/t)*100).toFixed(1):"0"}%</td><td style={{fontSize:10,color:"var(--t2)",maxWidth:300}}>{bd[cat.key]?.details||"—"}</td></tr>)})}</tbody>
      ...
      {result.negotiationPoints?.length>0&&(plan!=="free"?<div style={{background:"var(--s1)",border:"1px solid var(--s2)",borderRadius:8,padding:12,marginTop:14}}>
      ...
      </div>:<div className="upsell-locked" onClick={()=>window.__costlens_upgrade?.()}>
        <div style={{fontSize:9,textTransform:"uppercase",letterSpacing:.8,color:"var(--t3)",marginBottom:6}}>🎯 Negotiation Leverage ({result.negotiationPoints.length} points)</div>
        <div className="upsell-blur">{result.negotiationPoints.slice(0,1).map((p,i)=><div key={i} style={{fontSize:11,color:"var(--t2)",lineHeight:1.4}}>{p.slice(0,60)}...</div>)}</div>
        <div className="upsell-overlay">🔒 Upgrade to Pro to unlock negotiation leverage points<br/><span className="upsell-link">Upgrade → ₹1,999/mo</span></div>
      </div>)}
      {/* Benchmark Range */}
      ...
      <div style={{textAlign:"center",color:"var(--t3)",fontSize:8,marginTop:14,borderTop:"1px solid var(--s2)",paddingTop:8}}>Zero-Based Should-Cost Analysis • OH 15% on Process • Profit 8% • Confidential — {result.date}</div>
      {plan==="free"&&<div className="result-upgrade-cta" onClick={()=>window.__costlens_upgrade?.()}><span>⚡</span> <strong>Upgrade to Pro</strong> for volume discount slabs, negotiation leverage, PDF export, and 50 AI analyses/month <span className="upsell-link">→ ₹1,999/mo</span></div>}
      <div className="mod-actions"><button className="btn-sec" onClick={onBack}>← New Analysis</button><button className="btn-pri" onClick={()=>plan==="free"?window.__costlens_upgrade?.():window.print()}>{plan==="free"?"🔒 PDF (Pro)":"📄 Print/PDF"}</button></div>
    </div>);
  }
    ...
    return(<div className="result-view">
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:14,flexWrap:"wrap",gap:10}}>
        ...
        <div style={{background:"linear-gradient(135deg,rgba(139,92,246,.12),rgba(139,92,246,.04))",border:"1px solid rgba(139,92,246,.3)",borderRadius:9,padding:"10px 18px",textAlign:"center"}}>
          ...
          <div style={{fontSize:9,color:"var(--t3)"}}>Amort: ₹{(d.amortizedPerPiece||0).toFixed(2)}/pc</div>
        </div>
      </div>
      ...
      {result.ops?.length>0&&<div style={{marginTop:12}}><div style={{fontSize:10,fontWeight:600,color:"var(--t3)",textTransform:"uppercase",marginBottom:6}}>Operations ({result.ops.length} items)</div>
      ...
      {plan==="free"&&<div className="result-upgrade-cta" onClick={()=>window.__costlens_upgrade?.()}><span>⚡</span> <strong>Upgrade to Pro</strong> for PDF export, AI extraction, and 50 credits/month <span className="upsell-link">→ ₹1,999/mo</span></div>}
      <div className="mod-actions"><button className="btn-sec" onClick={onBack}>← New Analysis</button><button className="btn-pri" onClick={()=>plan==="free"?window.__costlens_upgrade?.():window.print()}>{plan==="free"?"🔒 PDF (Pro)":"📄 Print/PDF"}</button></div>
    </div>);
  }
  ...
  return(<div className="result-view">
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:14,flexWrap:"wrap",gap:10}}>
      ...
      <div style={{background:"linear-gradient(135deg,rgba(37,99,235,.12),rgba(37,99,235,.04))",border:"1px solid rgba(37,99,235,.3)",borderRadius:9,padding:"10px 18px",textAlign:"center"}}>
        ...
        <div style={{fontFamily:"'IBM Plex Mono'",fontSize:20,fontWeight:700,color:"var(--ac)"}}>{typeof result.resultVal==="object"?JSON.stringify(result.resultVal):result.resultVal}</div>
      </div>
    </div>
    ...
    {plan==="free"&&<div className="result-upgrade-cta" onClick={()=>window.__costlens_upgrade?.()}><span>⚡</span> <strong>Upgrade to Pro</strong> — PDF export, AI-powered analysis, and unlimited history <span className="upsell-link">→ ₹1,999/mo</span></div>}
    <div className="mod-actions"><button className="btn-sec" onClick={onBack}>← New Analysis</button><button className="btn-pri" onClick={()=>plan==="free"?window.__costlens_upgrade?.():window.print()}>{plan==="free"?"🔒 PDF (Pro)":"📄 Print/PDF"}</button></div></div>);
}

// ═══════════════════════════════════════════════════════════════
// ═══════════════════════════════════════════════════════════════
// AI TOOL RUNNER — Price Check, Contract, RFQ, Negotiation
// ═══════════════════════════════════════════════════════════════
function AIToolRunner({tool,credits,useCredit,user,onBack,plan,freeChecks,useFreeCheck}){
  const[fields,setFields]=useState({});const[files,setFiles]=useState([]);
  ...
  const[draftEmail,setDraftEmail]=useState(null);const[emailLoading,setEmailLoading]=useState(false);
  const[emailType,setEmailType]=useState(null);
  const setF=(k,v)=>setFields(p=>({...p,[k]:v}));
  const isFreeCheck=tool.id==="price-check"&&plan==="free"&&freeChecks>0;
  const canRun=isFreeCheck||credits>=tool.credits;

  const generateEmail=async(type)=>{
    if(credits<1&&plan!=="free"){setErr("Need 1 credit for email draft");return}
    setEmailLoading(true);setEmailType(type);setDraftEmail(null);
    try{
      if(plan!=="free")useCredit();
      const emailPrompts={
        "price-reduction":`You are a senior procurement professional writing to a supplier. Based on this price analysis:
${JSON.stringify(result)}
Item context: ${JSON.stringify(fields)}

Draft a professional but firm price reduction request email. Include:
- Reference the specific item and current quoted price
- State that your analysis shows the price is above market benchmark
- Provide the fair price range as supporting evidence
- Request a revised quotation within 5 working days
- Maintain professional relationship tone
- Keep it under 200 words

RESPOND ONLY VALID JSON: {"subject":"email subject","body":"email body with \\n for line breaks","tone":"Professional","recipientType":"Supplier Sales/Commercial"}`,

        "amendment-request":`You are a senior procurement professional writing to a supplier about contract amendments. Based on this contract analysis:
${JSON.stringify(result)}

Draft a professional amendment request email. Include:
- Reference the contract being reviewed
- List each risky clause that needs amendment with your suggested change
- List missing clauses that need to be added
- Request a revised draft within 7 working days
- Reference industry standard practices
- Keep it professional but firm

RESPOND ONLY VALID JSON: {"subject":"email subject","body":"email body with \\n for line breaks","tone":"Professional-Firm","recipientType":"Supplier Legal/Commercial"}`,

        "award-letter":`You are a senior procurement professional writing supplier award and regret letters. Based on this RFQ comparison:
${JSON.stringify(result)}

Draft TWO emails:
1. Award letter to the recommended vendor — congratulate, confirm pricing, state next steps (PO to follow, kick-off meeting)
2. Regret letter to non-selected vendors — thank them, state decision was based on techno-commercial evaluation, keep door open for future opportunities

RESPOND ONLY VALID JSON: {"awardEmail":{"subject":"","body":"","vendor":"winner name"},"regretEmail":{"subject":"","body":"","vendors":"other vendor names"},"tone":"Professional"}`,

        "cbs-request":`You are a senior procurement professional requesting a cost breakdown from a supplier. Based on this price analysis:
${JSON.stringify(result)}
Item: ${JSON.stringify(fields)}

Draft an email requesting the supplier to submit a detailed Cost Breakdown Sheet (CBS). Include:
- Reference the quoted item and price
- State that CBS submission is mandatory for quotation evaluation
- List required CBS elements: raw material (weight, grade, rate), conversion (process, cycle time, MHR), overhead, profit, logistics
- Specify format (Excel template preferred)
- Set deadline of 5 working days
- Mention that pricing will be benchmarked against should-cost analysis

RESPOND ONLY VALID JSON: {"subject":"email subject","body":"email body with \\n for line breaks","tone":"Professional","recipientType":"Supplier Sales"}`,

        "meeting-request":`You are a senior procurement professional requesting a negotiation meeting. Based on this negotiation strategy brief:
${JSON.stringify(result)}
Supplier context: ${JSON.stringify(fields)}

Draft a meeting request email to the supplier. Include:
- Request a face-to-face or virtual meeting for commercial discussion
- Mention the items/categories under discussion
- Suggest 2-3 possible dates (next week)
- Request they come with decision-making authority and latest pricing data
- Keep the tone collaborative but signal that you have data and alternatives
- Do NOT reveal your negotiation strategy

RESPOND ONLY VALID JSON: {"subject":"email subject","body":"email body with \\n for line breaks","tone":"Collaborative","recipientType":"Supplier Key Account Manager"}`,

        "escalation":`You are a senior procurement professional writing an internal escalation or management update. Based on this analysis:
Tool: ${tool.name}
Result: ${JSON.stringify(result)}
Context: ${JSON.stringify(fields)}

Draft a concise management update email (to your CPO/VP/MD). Include:
- Summary of what was analyzed
- Key findings (2-3 bullet points)
- Recommended action with expected impact in ₹
- Timeline and next steps
- Any decision/approval needed from management

RESPOND ONLY VALID JSON: {"subject":"email subject","body":"email body with \\n for line breaks","tone":"Executive Summary","recipientType":"Internal Management"}`
      };

      const prompt=emailPrompts[type];if(!prompt)throw new Error("Unknown email type");
      const r=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-5-20250929",max_tokens:2000,temperature:0.3,messages:[{role:"user",content:prompt}]})});
      if(!r.ok)throw new Error("API "+r.status);
      const d=await r.json(),txt=d.content?.map(c=>c.text||"").join("")||"";
      const i1=txt.indexOf("{"),i2=txt.lastIndexOf("}");
      if(i1>=0&&i2>i1)setDraftEmail(JSON.parse(txt.slice(i1,i2+1)));
      else throw new Error("No response");
    }catch(e){setErr("Email draft failed: "+e.message)}
    setEmailLoading(false);
  };

  const EmailDraftView=({email})=>{
    if(!email)return null;
    const isMulti=email.awardEmail;
    const emails=isMulti?[{label:"✅ Award Letter — "+email.awardEmail.vendor,...email.awardEmail},{label:"📋 Regret Letter — "+email.regretEmail.vendors,...email.regretEmail}]:[email];
    return(<div style={{marginTop:16}}>{emails.map((em,idx)=><div key={idx} style={{background:"var(--bg)",border:"1px solid var(--ac)",borderRadius:10,padding:16,marginBottom:10}}>
      {isMulti&&<div style={{fontSize:11,fontWeight:700,color:"var(--ac)",marginBottom:8}}>{em.label}</div>}
      <div style={{fontSize:10,color:"var(--t3)",marginBottom:2}}>Subject:</div>
      <div style={{fontSize:13,fontWeight:600,marginBottom:10,padding:"6px 10px",background:"var(--s1)",borderRadius:6}}>{em.subject}</div>
      <div style={{fontSize:10,color:"var(--t3)",marginBottom:2}}>Body:</div>
      <div style={{fontSize:12,lineHeight:1.7,whiteSpace:"pre-wrap",padding:"10px 12px",background:"var(--s1)",borderRadius:6,fontFamily:"DM Sans"}}>{(em.body||"").replace(/\\n/g,"\n")}</div>
      <div style={{display:"flex",gap:8,marginTop:10}}>
        <button className="btn-pri" style={{fontSize:11}} onClick={()=>{const txt="Subject: "+em.subject+"\n\n"+em.body.replace(/\\n/g,"\n");try{const ta=document.createElement("textarea");ta.value=txt;ta.style.cssText="position:fixed;left:-9999px";document.body.appendChild(ta);ta.select();document.execCommand("copy");document.body.removeChild(ta);alert("📋 Copied to clipboard!")}catch{try{navigator.clipboard.writeText(txt).then(()=>alert("📋 Copied!"))}catch{alert("Copy failed — please select and copy manually")}}}}>📋 Copy Email</button>
        <button className="btn-sec" style={{fontSize:11}} onClick={()=>{const body=em.body.replace(/\\n/g,"\r\n").replace(/\n/g,"\r\n");const url="mailto:?subject="+encodeURIComponent(em.subject)+"&body="+encodeURIComponent(body);window.location.href=url}}>📧 Open in Mail</button>
      </div>
    </div>)}</div>);
  };

  const SmartActions=({toolId})=>{
    const actions={
      "price-check":[
        {icon:"📧",label:"Draft Price Reduction Email",type:"price-reduction",desc:"AI drafts a professional email requesting price revision with your analysis data as evidence"},
        {icon:"📋",label:"Request CBS from Supplier",type:"cbs-request",desc:"Demand a cost breakdown sheet — the next step after finding price is high"},
        {icon:"📊",label:"Draft Management Update",type:"escalation",desc:"Send key findings to your CPO/VP with recommended action"},
      ],
      "contract-analyzer":[
        {icon:"📧",label:"Draft Amendment Request",type:"amendment-request",desc:"Email supplier with specific clause changes needed — citing each risk"},
        {icon:"🎯",label:"Generate Negotiation Brief",desc:"Create a negotiation strategy for this supplier",link:"negotiation-brief"},
        {icon:"📊",label:"Draft Management Update",type:"escalation",desc:"Escalate risky findings to management with recommendations"},
      ],
      "rfq-comparator":[
        {icon:"📧",label:"Draft Award + Regret Letters",type:"award-letter",desc:"AI generates both — award to winner, professional regret to others"},
        {icon:"💰",label:"Open TCO Comparison",desc:"Run deeper TCO analysis on shortlisted vendors",link:"tco"},
        {icon:"📊",label:"Draft Management Update",type:"escalation",desc:"Share comparison summary with decision-makers"},
      ],
      "negotiation-brief":[
        {icon:"📧",label:"Draft Meeting Request",type:"meeting-request",desc:"Request negotiation meeting — collaborative tone, no strategy revealed"},
        {icon:"📊",label:"Draft Management Update",type:"escalation",desc:"Share strategy and get buy-in before the meeting"},
        {icon:"📋",label:"Open CBS Analyzer",desc:"Validate supplier's cost breakdown before negotiation",link:"cbs"},
      ]
    };
    const acts=actions[toolId]||[];
    return(<div style={{marginTop:20,borderTop:"1px solid var(--s2)",paddingTop:16}}>
      <div style={{fontSize:13,fontWeight:700,marginBottom:10}}>⚡ Smart Actions — What's Next?</div>
      <div style={{display:"grid",gridTemplateColumns:"1fr",gap:8}}>
        {acts.map((a,i)=><div key={i} style={{background:"var(--s1)",border:"1px solid var(--s2)",borderRadius:8,padding:12,cursor:"pointer",transition:"all .15s"}} className="smart-action-card"
          onClick={()=>{if(a.type)generateEmail(a.type);else if(a.link){/* cross-tool navigation handled by parent */}}}>
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            <span style={{fontSize:18}}>{a.icon}</span>
            <div style={{flex:1}}>
              <div style={{fontSize:12,fontWeight:600}}>{a.label} {a.type&&<span style={{fontSize:9,color:"var(--ac)",fontWeight:400}}>(1 credit)</span>}</div>
      ...
      {emailLoading&&<div className="mod-loading" style={{marginTop:10}}>🤖 AI drafting email... 15 seconds</div>}
      <EmailDraftView email={draftEmail}/>
    </div>);
  };
  const runTool=async()=>{
    if(!canRun){setErr(`Need ${tool.credits} credits (you have ${credits}). Upgrade to Pro.`);return}
    if(tool.needsFile&&files.length===0){setErr("Please upload required files");return}
    setLoading(true);setErr("");
    try{
      if(isFreeCheck){useFreeCheck()}else{for(let i=0;i<tool.credits;i++)useCredit()}
      const promptTemplate=TOOL_PROMPTS[tool.id];
      const prompt=typeof promptTemplate==="function"?promptTemplate(fields):promptTemplate;
      const content=[];
      for(const f of files){const ext=fext(f);
        if(["xlsx","xls","csv","tsv"].includes(ext)){try{const data=await parseSpreadsheet(f);content.push({type:"text",text:`[DATA: ${f.name}]\n${data.slice(0,80000)}`})}catch{content.push({type:"text",text:`[File: ${f.name} — parse error]`})}}
      ...
      content.push({type:"text",text:prompt+(notes?"\n\nAdditional context: "+notes:"")});
      let r=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-5-20250929",max_tokens:8192,temperature:0,messages:[{role:"user",content}]})});
      if(!r.ok){const eb=await r.text().catch(()=>"");throw new Error("API "+r.status+(eb?": "+eb.slice(0,200):""))}
      const d=await r.json(),txt=d.content?.map(c=>c.text||"").join("")||"";
      const i1=txt.indexOf("{"),i2=txt.lastIndexOf("}");if(i1===-1||i2<=i1)throw new Error("No structured response");
      setResult(JSON.parse(txt.slice(i1,i2+1)));
    }catch(e){setErr("Analysis failed: "+e.message)}
    setLoading(false);
  };
  // ─── RESULT: PRICE CHECK ───
  if(result&&tool.id==="price-check")return(<div style={{maxWidth:700}}>
    <button className="btn-sec" style={{fontSize:11,marginBottom:12}} onClick={()=>setResult(null)}>← New Check</button>
    <div className="rpt-header"><span style={{fontSize:24}}>💲</span><div><div style={{fontSize:18,fontWeight:800,fontFamily:"'Bricolage Grotesque'"}}>Price Reasonableness Check</div><div style={{fontSize:10,color:"var(--t3)"}}>AI Analysis • {new Date().toLocaleDateString("en-IN")}</div></div></div>
    <div style={{background:result.verdict==="Reasonable"||result.verdict==="Low"?"rgba(16,185,129,.08)":result.verdict==="Very High"?"rgba(239,68,68,.08)":"rgba(245,158,11,.08)",border:"1px solid "+(result.verdict==="Reasonable"||result.verdict==="Low"?"rgba(16,185,129,.3)":result.verdict==="Very High"?"rgba(239,68,68,.3)":"rgba(245,158,11,.3)"),borderRadius:10,padding:16,marginBottom:14,textAlign:"center"}}>
      <div style={{fontSize:24,fontWeight:800,color:result.verdict==="Reasonable"?"var(--ok)":result.verdict==="Very High"?"var(--err)":"var(--warn)"}}>{result.verdict==="Reasonable"?"✅ PRICE APPEARS REASONABLE":result.verdict==="Low"?"💡 PRICE APPEARS LOW — VERIFY QUALITY":result.verdict==="Very High"?"🚩 VERY HIGH":"⚠️ PRICE APPEARS HIGH"}</div>
      <div style={{fontSize:12,color:"var(--t2)",marginTop:6}}>Confidence: {result.confidence} | Quoted: ₹{result.quotedPrice} | Fair Range: ₹{result.estimatedFairRange?.low}–₹{result.estimatedFairRange?.high}</div>
    </div>
    {result.breakdown&&<div className="rpt-section"><div className="rpt-section-title">📊 Cost Breakdown Estimate</div>{Object.entries(result.breakdown).map(([k,v],i)=><div key={i} style={{display:"flex",justifyContent:"space-between",padding:"8px 10px",background:i%2===0?"var(--s1)":"transparent",borderRadius:4,marginBottom:2}}><div><div style={{fontSize:12,fontWeight:600,textTransform:"capitalize"}}>{k}</div><div style={{fontSize:10,color:"var(--t3)"}}>{v.basis}</div></div><div style={{fontFamily:"IBM Plex Mono",fontWeight:700,fontSize:13,color:"var(--ac)"}}>{v.estimate}</div></div>)}</div>}
    {result.keyObservations?.length>0&&<div className="rpt-section"><div className="rpt-section-title">🔍 Key Observations</div>{result.keyObservations.map((o,i)=><div key={i} className="rpt-finding-item"><span className="rpt-num">{i+1}</span><span style={{fontSize:12}}>{o}</span></div>)}</div>}
    {result.suggestedActions?.length>0&&<div className="rpt-section"><div className="rpt-section-title">💡 Suggested Actions</div>{result.suggestedActions.map((a,i)=><div key={i} style={{background:"rgba(16,185,129,.04)",border:"1px solid rgba(16,185,129,.15)",borderRadius:6,padding:8,marginBottom:4,fontSize:12}}>{a}</div>)}</div>}
    {result.negotiationTip&&<div style={{background:"rgba(37,99,235,.06)",border:"1px solid rgba(37,99,235,.2)",borderRadius:8,padding:12,marginBottom:12}}><div style={{fontSize:10,fontWeight:700,color:"var(--ac)",textTransform:"uppercase",marginBottom:4}}>🎯 Negotiation Tip</div><div style={{fontSize:12}}>{result.negotiationTip}</div></div>}
    <SmartActions toolId="price-check"/>
    <div className="mod-actions" style={{marginTop:12}}><button className="btn-sec" onClick={()=>{setResult(null);setDraftEmail(null)}}>New Check</button><button className="btn-pri" onClick={()=>window.print()}>📄 Print/PDF</button></div>
  </div>);
  // ─── RESULT: CONTRACT ───
  if(result&&tool.id==="contract-analyzer")return(<div style={{maxWidth:750}}>
    <button className="btn-sec" style={{fontSize:11,marginBottom:12}} onClick={()=>setResult(null)}>← New Analysis</button>
    <div className="rpt-header"><span style={{fontSize:24}}>📜</span><div><div style={{fontSize:18,fontWeight:800,fontFamily:"'Bricolage Grotesque'"}}>Contract Clause Analysis</div><div style={{fontSize:10,color:"var(--t3)"}}>AI Analysis • {new Date().toLocaleDateString("en-IN")}</div></div></div>
    {result.contractSummary&&<div style={{background:"var(--s1)",border:"1px solid var(--s2)",borderRadius:8,padding:12,marginBottom:12,display:"grid",gridTemplateColumns:"1fr 1fr",gap:6,fontSize:11}}>{Object.entries(result.contractSummary).map(([k,v])=>v?<div key={k}><span style={{color:"var(--t3)",textTransform:"capitalize"}}>{k.replace(/([A-Z])/g," $1")}: </span><strong>{v}</strong></div>:null)}</div>}
    <div style={{background:result.overallRiskScore==="Low"?"rgba(16,185,129,.08)":result.overallRiskScore==="Critical"||result.overallRiskScore==="High"?"rgba(239,68,68,.08)":"rgba(245,158,11,.08)",borderRadius:8,padding:10,marginBottom:14,textAlign:"center",fontSize:14,fontWeight:700}}>Overall Risk: {result.overallRiskScore}</div>
    {result.favorableClauses?.length>0&&<div className="rpt-section"><div className="rpt-section-title">✅ Favorable ({result.favorableClauses.length})</div>{result.favorableClauses.map((c,i)=><div key={i} style={{background:"rgba(16,185,129,.04)",border:"1px solid rgba(16,185,129,.12)",borderRadius:6,padding:10,marginBottom:4}}><div style={{fontSize:12,fontWeight:600}}>{c.clause}</div><div style={{fontSize:11,color:"var(--t2)",marginTop:2}}>{c.detail}</div><div style={{fontSize:10,color:"var(--ok)",marginTop:2}}>✓ {c.benefit}</div></div>)}</div>}
    {result.riskyAndUnfavorableClauses?.length>0&&<div className="rpt-section"><div className="rpt-section-title">🚩 Risky ({result.riskyAndUnfavorableClauses.length})</div>{result.riskyAndUnfavorableClauses.map((c,i)=><div key={i} style={{background:"rgba(239,68,68,.04)",border:"1px solid rgba(239,68,68,.12)",borderRadius:6,padding:10,marginBottom:4}}><div style={{display:"flex",justifyContent:"space-between"}}><div style={{fontSize:12,fontWeight:600}}>{c.clause}</div><span className={`rpt-sev ${c.severity?.toLowerCase()}`}>{c.severity}</span></div><div style={{fontSize:11,color:"var(--t2)",marginTop:2}}>{c.detail}</div><div style={{fontSize:10,color:"var(--err)",marginTop:2}}>⚠️ {c.risk}</div>{c.amendment&&<div style={{fontSize:10,color:"var(--ac)",marginTop:4,background:"rgba(37,99,235,.06)",padding:6,borderRadius:4}}>📝 Suggested: {c.amendment}</div>}</div>)}</div>}
    {result.missingCriticalClauses?.length>0&&<div className="rpt-section"><div className="rpt-section-title">⚠️ Missing ({result.missingCriticalClauses.length})</div>{result.missingCriticalClauses.map((c,i)=><div key={i} style={{background:"rgba(245,158,11,.04)",borderRadius:6,padding:10,marginBottom:4}}><div style={{fontSize:12,fontWeight:600}}>{c.clause} <span style={{fontSize:9,color:"var(--warn)"}}>({c.importance})</span></div><div style={{fontSize:10,color:"var(--t2)"}}>Risk: {c.risk}</div>{c.suggestedText&&<div style={{fontSize:10,color:"var(--ac)",marginTop:4,background:"rgba(37,99,235,.06)",padding:6,borderRadius:4}}>📝 Add: {c.suggestedText}</div>}</div>)}</div>}
    {result.topRecommendations?.length>0&&<div className="rpt-section"><div className="rpt-section-title">💡 Recommendations</div>{result.topRecommendations.map((r,i)=><div key={i} style={{background:"var(--s1)",borderRadius:6,padding:8,marginBottom:4,fontSize:12}}><strong>{i+1}.</strong> {r}</div>)}</div>}
    <SmartActions toolId="contract-analyzer"/>
    <div className="mod-actions" style={{marginTop:12}}><button className="btn-sec" onClick={()=>{setResult(null);setDraftEmail(null)}}>New</button><button className="btn-pri" onClick={()=>window.print()}>📄 Print/PDF</button></div>
  </div>);
  // ─── RESULT: RFQ ───
  if(result&&tool.id==="rfq-comparator")return(<div style={{maxWidth:800}}>
    <button className="btn-sec" style={{fontSize:11,marginBottom:12}} onClick={()=>setResult(null)}>← New Comparison</button>
    <div className="rpt-header"><span style={{fontSize:24}}>📊</span><div><div style={{fontSize:18,fontWeight:800,fontFamily:"'Bricolage Grotesque'"}}>RFQ Techno-Commercial Comparison</div><div style={{fontSize:10,color:"var(--t3)"}}>AI Analysis • {new Date().toLocaleDateString("en-IN")}</div></div></div>
    {result.priceAnalysis&&<div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:8,marginBottom:14}}>{[["L1",result.priceAnalysis.l1Vendor,"var(--ok)"],["Lowest","₹"+result.priceAnalysis.lowestPrice,"var(--ok)"],["Avg","₹"+result.priceAnalysis.averagePrice,""],["Spread",result.priceAnalysis.priceSpread,"var(--warn)"]].map(([l,v,c],i)=><div key={i} style={{background:"var(--s1)",borderRadius:8,padding:10,textAlign:"center"}}><div style={{fontSize:9,color:"var(--t3)"}}>{l}</div><div style={{fontFamily:"IBM Plex Mono",fontSize:14,fontWeight:700,color:c||"var(--t1)"}}>{v}</div></div>)}</div>}
    {result.vendors?.length>0&&<div className="rpt-section"><div className="rpt-section-title">📋 Vendor Details</div>{result.vendors.map((v,i)=><div key={i} style={{background:"var(--s1)",borderRadius:8,padding:12,marginBottom:6}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}><div style={{fontSize:14,fontWeight:700}}>{v.name}</div><div style={{fontFamily:"IBM Plex Mono",fontSize:16,fontWeight:700,color:"var(--ac)"}}>₹{v.unitPrice}</div></div><div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:4,fontSize:10,color:"var(--t2)"}}><span>Lead: {v.leadTimeDays}d</span><span>Payment: {v.paymentTerms}</span><span>Warranty: {v.warranty}</span></div>{v.keyDeviations?.length>0&&<div style={{marginTop:6}}>{v.keyDeviations.map((d,di)=><span key={di} style={{display:"inline-block",fontSize:9,background:"rgba(239,68,68,.08)",color:"var(--err)",padding:"2px 6px",borderRadius:4,marginRight:4}}>⚠️ {d}</span>)}</div>}</div>)}</div>}
    {result.technoCommercialRanking?.length>0&&<div className="rpt-section"><div className="rpt-section-title">🏆 Ranking</div>{result.technoCommercialRanking.map((r,i)=><div key={i} style={{display:"flex",gap:10,alignItems:"center",padding:"8px 10px",background:i===0?"rgba(16,185,129,.06)":"var(--s1)",borderRadius:6,marginBottom:4}}><div style={{fontSize:18,fontWeight:800,color:i===0?"var(--ok)":"var(--t3)"}}>#{r.rank}</div><div style={{flex:1}}><div style={{fontSize:13,fontWeight:600}}>{r.vendor}{i===0&&" ✅"}</div><div style={{fontSize:10,color:"var(--t2)"}}>{r.rationale}</div></div><div style={{fontFamily:"IBM Plex Mono",fontWeight:700,color:"var(--ac)"}}>{r.score}/100</div></div>)}</div>}
    {result.recommendation&&<div style={{background:"rgba(37,99,235,.06)",borderRadius:8,padding:14,marginBottom:12}}><div style={{fontSize:12,fontWeight:700,color:"var(--ac)",marginBottom:4}}>🎯 Recommendation: {result.recommendation.bestValue}</div><div style={{fontSize:11,color:"var(--t2)"}}>{result.recommendation.rationale}</div></div>}
    <SmartActions toolId="rfq-comparator"/>
    <div className="mod-actions" style={{marginTop:12}}><button className="btn-sec" onClick={()=>{setResult(null);setDraftEmail(null)}}>New</button><button className="btn-pri" onClick={()=>window.print()}>📄 Print/PDF</button></div>
  </div>);
  // ─── RESULT: NEGOTIATION BRIEF ───
  if(result&&tool.id==="negotiation-brief")return(<div style={{maxWidth:700}}>
    <button className="btn-sec" style={{fontSize:11,marginBottom:12}} onClick={()=>setResult(null)}>← New Brief</button>
    <div className="rpt-header"><span style={{fontSize:24}}>🎯</span><div><div style={{fontSize:18,fontWeight:800,fontFamily:"'Bricolage Grotesque'"}}>{result.briefTitle||"Negotiation Strategy Brief"}</div><div style={{fontSize:10,color:"var(--t3)"}}>CONFIDENTIAL • {new Date().toLocaleDateString("en-IN")}</div></div></div>
    {result.supplierAssessment&&<div className="rpt-section"><div className="rpt-section-title">🏢 Supplier Assessment</div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>{Object.entries(result.supplierAssessment).map(([k,v])=><div key={k} style={{background:"var(--s1)",borderRadius:6,padding:8}}><div style={{fontSize:9,color:"var(--t3)",textTransform:"capitalize"}}>{k.replace(/([A-Z])/g," $1")}</div><div style={{fontSize:11}}>{v}</div></div>)}</div></div>}
    {result.leverageAnalysis?.length>0&&<div className="rpt-section"><div className="rpt-section-title">💪 Leverage Points</div>{result.leverageAnalysis.map((l,i)=><div key={i} style={{display:"flex",gap:8,padding:"8px 10px",background:"var(--s1)",borderRadius:6,marginBottom:4}}><span style={{fontSize:10,fontWeight:700,padding:"2px 6px",borderRadius:4,background:l.strength==="Strong"?"rgba(16,185,129,.15)":"rgba(245,158,11,.15)",color:l.strength==="Strong"?"var(--ok)":"var(--warn)",whiteSpace:"nowrap"}}>{l.strength}</span><div><div style={{fontSize:12,fontWeight:600}}>{l.point}</div><div style={{fontSize:10,color:"var(--t2)"}}>{l.howToUse}</div></div></div>)}</div>}
    {result.strategy&&<div className="rpt-section"><div className="rpt-section-title">🎯 Strategy</div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:10}}>{[["Approach",result.strategy.approach],["Opening",result.strategy.openingPosition],["Target",result.strategy.targetOutcome],["Walk-Away",result.strategy.walkAway]].map(([l,v],i)=><div key={i} style={{background:i===3?"rgba(239,68,68,.06)":"var(--s1)",borderRadius:6,padding:8}}><div style={{fontSize:9,color:i===3?"var(--err)":"var(--t3)",textTransform:"uppercase"}}>{l}</div><div style={{fontSize:11,fontWeight:i===0?700:400}}>{v}</div></div>)}</div>
      {result.strategy.concessionPlan?.length>0&&<>{result.strategy.concessionPlan.map((c,i)=><div key={i} style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6,padding:"6px 8px",background:i%2===0?"var(--s1)":"transparent",borderRadius:4,fontSize:10}}><div><span style={{color:"var(--err)"}}>Give:</span> {c.give}</div><div><span style={{color:"var(--ok)"}}>Get:</span> {c.get}</div></div>)}</>}
    </div>}
    {result.talkingPoints?.length>0&&<div className="rpt-section"><div className="rpt-section-title">🗣️ Talking Points</div>{result.talkingPoints.map((t,i)=><div key={i} style={{background:"var(--s1)",borderRadius:8,padding:10,marginBottom:6}}><div style={{fontSize:12,fontWeight:600,marginBottom:4}}>{t.point}</div><div style={{fontSize:10,color:"var(--ac)"}}>📊 {t.data}</div><div style={{fontSize:10,color:"var(--warn)"}}>🔄 Counter: {t.expectedResponse}</div><div style={{fontSize:10,color:"var(--ok)"}}>↩️ Rebuttal: {t.rebuttal}</div></div>)}</div>}
    {result.redLines?.length>0&&<div className="rpt-section"><div className="rpt-section-title">🚫 Red Lines</div>{result.redLines.map((r,i)=><div key={i} style={{background:"rgba(239,68,68,.06)",borderRadius:6,padding:8,marginBottom:4,fontSize:12,fontWeight:600,color:"var(--err)"}}>{r}</div>)}</div>}
    {result.riskMitigation&&<div style={{background:"rgba(245,158,11,.06)",borderRadius:8,padding:12,marginBottom:12}}><div style={{fontSize:10,fontWeight:700,color:"var(--warn)",marginBottom:4}}>📋 Plan B</div><div style={{fontSize:12}}>{result.riskMitigation}</div></div>}
    <SmartActions toolId="negotiation-brief"/>
    <div className="mod-actions" style={{marginTop:12}}><button className="btn-sec" onClick={()=>{setResult(null);setDraftEmail(null)}}>New</button><button className="btn-pri" onClick={()=>window.print()}>📄 Print/PDF</button></div>
  </div>);
  // ─── INPUT VIEW ───
  return(<div style={{maxWidth:700}}>
    <button className="btn-sec" style={{marginBottom:12,fontSize:11}} onClick={onBack}>← All AI Tools</button>
    <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:14}}><span style={{fontSize:28}}>{tool.icon}</span><div><div style={{fontSize:18,fontWeight:700}}>{tool.name}</div><div style={{fontSize:12,color:"var(--t2)"}}>{tool.desc}</div></div></div>
    <div style={{fontSize:10,color:"var(--t3)",marginBottom:14}}>{isFreeCheck?`🎁 ${freeChecks} free check${freeChecks>1?"s":""} remaining`:`Uses ${tool.credits} AI credit${tool.credits>1?"s":""} • ${credits} available`}</div>
    {tool.fields&&<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14}}>{tool.fields.map(f=><div key={f.k} style={{gridColumn:f.k==="leverage"||f.k==="constraints"||f.k==="history"||f.k==="itemDesc"?"1/-1":undefined}}>
      <label style={{fontSize:11,fontWeight:600,marginBottom:3,display:"block"}}>{f.l}</label>
      {f.t==="select"?<select value={fields[f.k]||""} onChange={e=>setF(f.k,e.target.value)} style={{width:"100%",padding:"8px 10px",borderRadius:6,border:"1px solid var(--s3)",background:"var(--bg)",color:"var(--t1)",fontSize:12}}><option value="">— Select —</option>{f.opts.map(o=><option key={o} value={o}>{o}</option>)}</select>
      :f.k==="leverage"||f.k==="constraints"||f.k==="history"?<textarea value={fields[f.k]||""} onChange={e=>setF(f.k,e.target.value)} placeholder={f.ph||""} style={{width:"100%",padding:"8px 10px",borderRadius:6,border:"1px solid var(--s3)",background:"var(--bg)",color:"var(--t1)",fontSize:12,minHeight:60,resize:"vertical",fontFamily:"DM Sans"}}/>
      :<input type={f.t||"text"} value={fields[f.k]||""} onChange={e=>setF(f.k,e.target.value)} placeholder={f.ph||""} style={{width:"100%",padding:"8px 10px",borderRadius:6,border:"1px solid var(--s3)",background:"var(--bg)",color:"var(--t1)",fontSize:12}}/>}
    </div>)}</div>}
    {tool.needsFile&&<><div style={{fontSize:11,fontWeight:600,marginBottom:4}}>{tool.fileHint}</div><FileUpload files={files} onChange={setFiles}/></>}
    <textarea className="mod-notes" value={notes} onChange={e=>setNotes(e.target.value)} placeholder="Additional context or special instructions..."/>
    {err&&<div className="mod-err">{err}</div>}
    {loading&&<div className="mod-loading">🤖 AI analyzing... This may take 30-60 seconds.</div>}
    <button className="btn-pri" style={{width:"100%",padding:"12px",fontSize:14,marginTop:8}} onClick={runTool} disabled={loading||!canRun}>{loading?"⏳ Analyzing...":!canRun?`🔒 Need ${tool.credits} credits — Upgrade to Pro`:isFreeCheck?`🎁 Run Free Check (${freeChecks} left)`:`⚡ Run Analysis (${tool.credits} credit${tool.credits>1?"s":""})`}</button>
  </div>);
}
...
const REPORT_FIELDS={
  ...
  "price-variance":[
    {key:"material",label:"Material / Item Code",required:true,hint:"Item code or material number"},
    {key:"description",label:"Material Description",required:true,hint:"Item name or description"},
    {key:"supplier",label:"Supplier Name",required:true,hint:"Vendor / Party name"},
    {key:"unitPrice",label:"Unit Price (₹)",required:true,hint:"Rate per unit / per kg"},
    {key:"date",label:"PO / GRN Date",required:true,hint:"Date for period comparison"},
    {key:"quantity",label:"Quantity",required:false,hint:"Order qty"},
    {key:"uom",label:"UOM",required:false,hint:"Unit of measure (Kg/Nos/Mtr)"},
    {key:"plant",label:"Plant / Location",required:false,hint:"Factory / warehouse"},
    {key:"category",label:"Material Group",required:false,hint:"Category / group code"},
  ],
  "inventory-health":[
    {key:"material",label:"Material Code",required:true,hint:"Item code / material number"},
    {key:"description",label:"Description",required:true,hint:"Item name"},
    {key:"stockQty",label:"Current Stock Qty",required:true,hint:"Quantity on hand"},
    {key:"stockValue",label:"Stock Value (₹)",required:true,hint:"Current inventory value"},
    {key:"lastReceipt",label:"Last Receipt Date",required:false,hint:"Last GRN / inward date"},
    {key:"lastIssue",label:"Last Issue Date",required:false,hint:"Last consumption date"},
    {key:"avgConsumption",label:"Avg Monthly Consumption",required:false,hint:"Monthly usage qty"},
    {key:"category",label:"Material Group",required:false,hint:"Category / group code"},
    {key:"plant",label:"Plant / Warehouse",required:false,hint:"Storage location"},
    {key:"reorderLevel",label:"Reorder Level",required:false,hint:"Min stock / ROL"},
  ],
  "supplier-scorecard":[
    {key:"supplier",label:"Supplier Name",required:true,hint:"Vendor / Party name"},
    {key:"material",label:"Material / Item",required:true,hint:"Item supplied"},
    {key:"poDate",label:"PO Date / Order Date",required:false,hint:"When order was placed"},
    {key:"promisedDate",label:"Promised Delivery Date",required:false,hint:"Committed date"},
    {key:"actualDate",label:"Actual Delivery / GRN Date",required:false,hint:"When received"},
    {key:"qtyOrdered",label:"Qty Ordered",required:false,hint:"PO quantity"},
    {key:"qtyReceived",label:"Qty Received / Accepted",required:false,hint:"GRN quantity"},
  ...
  "category-opportunity":[
    {key:"category",label:"Category / Material Group",required:true,hint:"Spend category name"},
    {key:"supplier",label:"Supplier Name",required:true,hint:"Vendor / Party name"},
    {key:"amount",label:"Spend Amount (₹)",required:true,hint:"Total value"},
    {key:"description",label:"Material / Item",required:false,hint:"Item description"},
    {key:"contractType",label:"Contract Type",required:false,hint:"Spot / Annual / LTA"},
    {key:"date",label:"PO / GRN Date",required:false,hint:"Date column"},
    {key:"plant",label:"Plant / Location",required:false,hint:"Factory / warehouse"},
    {key:"quantity",label:"Quantity",required:false,hint:"Order qty"},
  ],
  "cost-reduction-tracker":[
    {key:"initiative",label:"Initiative / Project Name",required:true,hint:"CR initiative name"},
    {key:"category",label:"Category",required:true,hint:"Material group / type"},
    {key:"baselinePrice",label:"Baseline Price (₹)",required:true,hint:"Old / before price"},
    {key:"newPrice",label:"New / Current Price (₹)",required:true,hint:"After negotiation price"},
    {key:"quantity",label:"Annual Qty",required:false,hint:"Yearly volume"},
    {key:"supplier",label:"Supplier Name",required:false,hint:"Vendor"},
    {key:"date",label:"Effective Date",required:false,hint:"When saving started"},
    {key:"owner",label:"Buyer / Owner",required:false,hint:"Who owns the initiative"},
    {key:"status",label:"Status",required:false,hint:"Approved / In-progress / Planned"},
  ],
  "savings-validation":[
    {key:"initiative",label:"Initiative Name",required:true,hint:"Savings initiative ID/name"},
    {key:"claimedSaving",label:"Claimed Saving (₹)",required:true,hint:"Reported savings value"},
    {key:"category",label:"Category",required:true,hint:"Material group / type"},
    {key:"baselinePrice",label:"Baseline Price",required:false,hint:"Reference / old price"},
    {key:"currentPrice",label:"Current Price",required:false,hint:"Actual price paid"},
    {key:"quantity",label:"Volume / Qty",required:false,hint:"Qty for calculation"},
    {key:"supplier",label:"Supplier",required:false,hint:"Vendor name"},
    {key:"method",label:"Savings Method",required:false,hint:"Negotiation / Substitution / etc"},
    {key:"approvedBy",label:"Approved By",required:false,hint:"Who validated"},
    {key:"date",label:"Period / Date",required:false,hint:"When saving was realized"},
  ],
  "supplier-risk":[
    {key:"supplier",label:"Supplier Name",required:true,hint:"Vendor / Party name"},
    {key:"amount",label:"Annual Spend (₹)",required:true,hint:"Total spend with supplier"},
    {key:"material",label:"Material / Items Supplied",required:true,hint:"Key items from this supplier"},
    {key:"location",label:"Supplier Location",required:false,hint:"City / State / Country"},
    {key:"alternates",label:"Alternate Sources",required:false,hint:"No. of alternates or names"},
    {key:"leadTime",label:"Lead Time (days)",required:false,hint:"Typical delivery time"},
    {key:"qualityScore",label:"Quality Score / PPM",required:false,hint:"Rejection rate or score"},
    {key:"deliveryScore",label:"Delivery OTIF %",required:false,hint:"On-time performance"},
    {key:"contractExpiry",label:"Contract Expiry",required:false,hint:"When contract ends"},
  ],
};
...
function DataMapper({files,fields,onMapped,onBack}){
    ...
    (async()=>{
      ...
      try{
        ...
        if(ext==="csv"||ext==="tsv"){
          ...
          const hdr=lines[0].split(ext==="tsv"?"\t":",").map(h=>h.replace(/^"|"$/g,"").trim());
          const dataRows=lines.slice(1).map(l=>l.split(ext==="tsv"?"\t":",").map(c=>c.replace(/^"|"$/g,"").trim()));
        ...
        setRows(best.rows);
        setTotalRows(best.totalRows);
        // Auto-detect columns
        const auto={};const hdrLow=best.headers.map(h=>h.toLowerCase());
        const patterns={supplier:/vendor|supplier|party|creditor/i,amount:/amount|value|total|net.?val|po.?val|spend/i,description:/desc|material|item|product|part/i,poNumber:/po.?n|order.?n|document/i,poDate:/date|posting|doc.?date|po.?date|grn.?date/i,date:/date|posting|doc.?date|period/i,quantity:/qty|quantity|quant/i,unitPrice:/price|rate|unit.?p|per.?unit/i,category:/group|categ|class|type|mat.?gr/i,plant:/plant|location|warehouse|site|stor/i,material:/material|item.?code|mat.?no|part.?no/i,stockQty:/stock.?qty|on.?hand|balance.?qty|avail/i,stockValue:/stock.?val|inv.?val|book.?val/i,lastReceipt:/last.?rec|last.?grn|last.?in/i,lastIssue:/last.?iss|last.?con|last.?out/i,avgConsumption:/avg|average|monthly.?con/i,promisedDate:/promis|commit|due.?date|delivery.?date|expect/i,actualDate:/actual|grn.?date|receipt.?date|received/i,qtyOrdered:/qty.?ord|order.?qty|po.?qty/i,qtyReceived:/qty.?rec|received|accepted|grn.?qty/i,qtyRejected:/reject|defect|rej.?qty/i,rejectionReason:/reason|defect.?type|rejection.?reason/i,contractType:/contract|spot|annual|lta/i,initiative:/initiative|project|cr.?name|saving.?id/i,baselinePrice:/baseline|old.?price|before|base.?rate|ref.?price/i,newPrice:/new.?price|current|after|revised/i,claimedSaving:/claimed|saving|benefit/i,currentPrice:/current|new|actual.?price/i,method:/method|lever|type|mechanism/i,owner:/buyer|owner|responsible/i,status:/status|stage|progress/i,approvedBy:/approved|validated|signed/i,location:/city|state|country|location|address/i,alternates:/alternate|alt.?source|backup/i,leadTime:/lead.?time|lt|delivery.?days/i,qualityScore:/quality|ppm|reject.?rate|score/i,deliveryScore:/otif|on.?time|delivery.?%/i,contractExpiry:/expir|end.?date|contract.?end|renewal/i,reorderLevel:/reorder|rol|min.?stock|safety/i,uom:/uom|unit.?of|measure/i};
        const fieldKeys=fields.map(f=>f.key);
        for(const[field,rx] of Object.entries(patterns)){
          if(!fieldKeys.includes(field))continue;
  ...
  const setMap=(field,colIdx)=>{
    const m={...mapping};
    if(colIdx===-1)delete m[field];else m[field]=colIdx;
    setMapping(m);
  };
  ...
  const buildCleanData=()=>{
    ...
    const dataRows=rows.map(r=>mapped.map(f=>{
  ...
  return(<div style={{maxWidth:750}}>
    <button className="btn-sec" style={{marginBottom:12,fontSize:11}} onClick={onBack}>← Back to Upload</button>
...
function ReportRunner({report,credits,useCredit,user,onBack}){
  ...
  const handleMapped=(csv,summary)=>{
    setCleanData(csv);setDataSummary(summary);setStatus("mapped");
  };
...
Each finding must include a specific metric/number extracted from the data.
...
Each must have: the finding, quantified business impact in ₹, and specific recommended action. Name actual items, suppliers, categories from the data.

### 4. RISK AREAS (5-8 critical risks)
Rate severity as High/Medium/Low with specific mitigation.

### 5. STRATEGIC RECOMMENDATIONS (6-10 recommendations)
Each: what to do, timeline (30/60/90/180 days), expected ₹ impact, effort level.

### 6. QUICK WINS (5-8 actions for next 30 days)
Low-effort, high-impact actions. Include specific items/suppliers/categories.
...
{"title":"","executiveSummary":["finding with ₹ metric"],"keyMetrics":[{"metric":"","value":"","trend":"up|down|stable","category":"cost|efficiency|risk|opportunity"}],"findings":[{"rank":1,"finding":"","impact":"₹X Lakhs","action":""}],"riskAreas":[{"risk":"","severity":"High|Medium|Low","recommendation":"","impact":""}],"recommendations":[{"recommendation":"","timeline":"30 days","expectedImpact":"₹X Lakhs","effort":"Low|Medium|High"}],"quickWins":[{"action":"","effort":"Low","impact":"₹X Lakhs","category":""}],"negotiationPoints":["data-backed leverage point"]}`;
      ...
      let r=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-5-20250929",max_tokens:8192,temperature:0,messages:[{role:"user",content:fileContent}]})});
      if(!r.ok){const errBody=await r.text().catch(()=>"");throw new Error("API "+r.status+(errBody?": "+errBody.slice(0,200):""))}
      const d=await r.json(),txt=d.content?.map(c=>c.text||"").join("")||"";
      const i1=txt.indexOf("{"),i2=txt.lastIndexOf("}");if(i1===-1||i2<=i1)throw new Error("No JSON in response");
      const res=JSON.parse(txt.slice(i1,i2+1));
  ...
  const requestExpert=async()=>{
    if(files.length===0){setErr("Please upload your data file first");return}
    setExpertSent(true);
    try{const requests=await dbGet("cl-expert-requests")||[];
      requests.push({id:Date.now(),report:report.id,reportName:report.name,user:user.email,userName:user.name,notes,fileNames:files.map(f=>f.name),date:new Date().toISOString(),status:"pending",price:report.expertPrice});
  ...
  if(status==="upload")return(<div style={{maxWidth:700}}>
    ...
    <div className="rpt-paths">
      <div className="rpt-path rpt-path-ai">
        ...
        {credits>=report.credits?<button className="btn-pri" onClick={()=>{if(files.length===0){setErr("Please upload your data file");return}if(REPORT_FIELDS[report.id]){setStatus("mapping")}else{runAI()}}} disabled={loading||files.length===0}>{REPORT_FIELDS[report.id]?"Map Columns & Analyze →":"Generate AI Report →"}</button>
        :<button className="upsell-btn" onClick={()=>window.__costlens_upgrade?.()}>🔒 Upgrade to Pro — {report.credits} credits needed</button>}
      </div>
      <div className="rpt-path rpt-path-expert">
        ...
        {expertSent?<div style={{background:"rgba(16,185,129,.08)",border:"1px solid rgba(16,185,129,.3)",borderRadius:6,padding:8,fontSize:11,color:"var(--ok)"}}>✅ Request submitted! We'll reach out at <strong>{user.email}</strong> within {report.turnaround}.</div>
    ...
    {RPT_SNIP[report.id]&&<div className="rpt-snippet">
      <div style={{fontSize:12,fontWeight:700,marginBottom:10,display:"flex",alignItems:"center",gap:6}}><span style={{fontSize:14}}>📄</span> Sample Report Preview</div>
      {/* Executive Summary snippet */}
      <div className="snip-section"><div className="snip-title">📌 Executive Summary</div>
        {RPT_SNIP[report.id].summary.map((s,i)=><div key={i} className="snip-es"><span className="snip-num">{i+1}</span><span>{s}</span></div>)}
      </div>
      {/* Key Metrics snippet */}
      <div className="snip-section"><div className="snip-title">📊 Key Metrics</div>
        <div className="snip-mg">{RPT_SNIP[report.id].metrics.map((m,i)=><div key={i} className="snip-mc">
          <div className="snip-mc-l">{m.m}</div>
          <div className="snip-mc-v">{m.v}</div>
          <div className={`snip-mc-t ${m.t}`}>{m.t==="down"?"↓ Favorable":m.t==="up"?"↑ Attention":"— Stable"}</div>
        </div>)}</div>
      </div>
      {/* Top Findings snippet */}
      <div className="snip-section"><div className="snip-title">🔍 Top Findings</div>
        {RPT_SNIP[report.id].findings.map((f,i)=><div key={i} className="snip-finding">
          <div className="snip-f-rank">#{i+1}</div>
          <div style={{flex:1}}><div className="snip-f-text">{f.f}</div><div className="snip-f-action">→ {f.a}</div></div>
          <div className="snip-f-impact">{f.i}</div>
        </div>)}
      </div>
      {/* Risk Areas snippet */}
      <div className="snip-section"><div className="snip-title">⚠️ Risk Flags</div>
        {RPT_SNIP[report.id].risks.map((r,i)=><div key={i} className="snip-risk">
          <span className={`snip-sev ${r.s.toLowerCase()}`}>{r.s}</span>
          <span>{r.r}</span>
        </div>)}
      </div>
      <div style={{textAlign:"center",fontSize:9,color:"var(--t3)",marginTop:8,fontStyle:"italic"}}>This is a sample preview with illustrative data — your actual report will be generated from your uploaded data</div>
    </div>}
  </div>);

  if(status==="mapping")return(<DataMapper files={files} fields={REPORT_FIELDS[report.id]||[]} onMapped={handleMapped} onBack={()=>setStatus("upload")}/>);

  if(status==="mapped"&&dataSummary)return(<div style={{maxWidth:700}}>
    <button className="btn-sec" style={{marginBottom:12,fontSize:11}} onClick={()=>setStatus("mapping")}>← Back to Column Mapping</button>
  ...
  // ─── REPORT SMART ACTIONS ───
  const RPT_ACTIONS={
    "spend":[
      {icon:"📧",label:"Supplier Consolidation Notice",type:"rpt-spend-consolidation",desc:"Inform tail-spend suppliers of upcoming consolidation — professional but firm"},
      {icon:"📊",label:"CPO Strategy Update",type:"rpt-spend-mgmt",desc:"Spend insights + savings roadmap email for CPO/management review"},
      {icon:"🎯",label:"Open Category Opportunity Analysis",desc:"Deep-dive into category-level savings potential",link:"category-opportunity"},
    ],
    "price-variance":[
      {icon:"📧",label:"Price Correction Letter",type:"rpt-pv-correction",desc:"Demand price rollback from suppliers with unjustified increases — with data evidence"},
      {icon:"📊",label:"Price Movement Alert to Management",type:"rpt-pv-mgmt",desc:"Flag top variance items to CPO with recommended actions"},
      {icon:"🎯",label:"Generate Negotiation Brief",desc:"Build negotiation strategy for price correction meetings",link:"negotiation-brief"},
    ],
    "inventory-health":[
      {icon:"📧",label:"NMS Liquidation / Return Request",type:"rpt-inv-nms",desc:"Email suppliers/stores with dead stock list and required action (return/scrap/liquidate)"},
      {icon:"📊",label:"Working Capital Update to CFO",type:"rpt-inv-cfo",desc:"Quantify blocked capital with recovery plan and timeline"},
      {icon:"🎯",label:"Run Supplier Scorecard",desc:"Check if delivery failures are causing inventory build-up",link:"supplier-scorecard"},
    ],
    "supplier-scorecard":[
      {icon:"📧",label:"Performance Warning Letter",type:"rpt-ss-warning",desc:"Formal warning to Red/Amber suppliers with improvement targets and timeline"},
      {icon:"📧",label:"Supplier Appreciation Letter",type:"rpt-ss-appreciation",desc:"Recognize Green-rated suppliers — builds relationship for future negotiations"},
      {icon:"🎯",label:"Generate Negotiation Brief",desc:"Build strategy for performance-linked price discussion",link:"negotiation-brief"},
    ],
    "category-opportunity":[
      {icon:"📧",label:"RFQ Launch / Market Inquiry",type:"rpt-cat-rfq",desc:"Invite new suppliers for competitive bidding on high-opportunity categories"},
      {icon:"📊",label:"Savings Pipeline Update",type:"rpt-cat-mgmt",desc:"Management email with opportunity ranking, timeline, and resource needs"},
      {icon:"🎯",label:"Generate Negotiation Brief",desc:"Build strategy for incumbent supplier renegotiation",link:"negotiation-brief"},
    ],
    "cost-reduction-tracker":[
      {icon:"📧",label:"Initiative Follow-up Email",type:"rpt-cr-followup",desc:"Chase stuck/delayed initiatives — politely firm with deadline and escalation path"},
      {icon:"📊",label:"Savings Dashboard — MD/Board Update",type:"rpt-cr-board",desc:"Pipeline vs target summary for board presentation email"},
      {icon:"🎯",label:"Run Savings Validation",desc:"Get CFO-ready validated savings from your pipeline data",link:"savings-validation"},
    ],
    "savings-validation":[
      {icon:"📧",label:"Finance Alignment Request",type:"rpt-sv-finance",desc:"Get CFO/finance sign-off on methodology and validated numbers"},
      {icon:"📊",label:"Board-Ready Savings Report",type:"rpt-sv-board",desc:"Concise email with validated savings for board/MD presentation"},
      {icon:"🎯",label:"Open Cost Reduction Tracker",desc:"Track and manage your savings pipeline",link:"cost-reduction-tracker"},
    ],
    "supplier-risk":[
      {icon:"📧",label:"Dual-Source Development RFQ",type:"rpt-risk-rfq",desc:"Invite alternate suppliers for single-source high-risk items"},
      {icon:"📧",label:"BCP / Risk Escalation to Management",type:"rpt-risk-mgmt",desc:"Business continuity alert with risk heat map and mitigation plan"},
      {icon:"🎯",label:"Run Supplier Scorecard",desc:"Assess performance of at-risk suppliers",link:"supplier-scorecard"},
    ],
  };

  const genReportEmail=async(type)=>{
    if(credits<1){setErr("Need 1 credit for email draft");return}
    setEmailLoading(true);setDraftEmail(null);
    try{
      useCredit();
      const ctx=JSON.stringify({report:report.name,findings:result.executiveSummary,metrics:result.keyMetrics,recommendations:result.recommendations,quickWins:result.quickWins,risks:result.riskAreas,negotiation:result.negotiationPoints});
      const prompts={
"rpt-spend-consolidation":`You are a GCPO writing to tail-spend suppliers about upcoming vendor consolidation. Based on spend analysis:
${ctx}
Draft a professional email informing supplier(s) that their category is being consolidated. Include: rationale (efficiency, better terms), what it means for them (may need to compete in upcoming RFQ), timeline, and that their past service is valued. Firm but respectful.
RESPOND ONLY VALID JSON: {"subject":"","body":"email body with \\n for line breaks"}`,

"rpt-spend-mgmt":`You are a CPO writing to MD/VP about spend analysis findings. Based on:
${ctx}
Draft executive update email: total spend overview, top 3 findings with ₹ impact, recommended savings roadmap (quick wins + structural), resource needs, and next steps. Concise, data-driven.
RESPOND ONLY VALID JSON: {"subject":"","body":"email body with \\n for line breaks"}`,

"rpt-pv-correction":`You are a senior procurement manager writing to suppliers who increased prices without commodity justification. Based on price variance analysis:
${ctx}
Draft a price correction demand letter: reference specific items with unjustified increases, cite commodity index data showing no market justification, demand price rollback to previous level, set 7-day deadline for revised pricing. Professional but uncompromising.
RESPOND ONLY VALID JSON: {"subject":"","body":"email body with \\n for line breaks"}`,

"rpt-pv-mgmt":`You are CPO alerting management about significant price movements. Based on:
${ctx}
Draft price movement alert: top 10 items by variance impact, which are justified (commodity-linked) vs unjustified, total P&L impact in ₹, recommended corrective actions, and escalations needed.
RESPOND ONLY VALID JSON: {"subject":"","body":"email body with \\n for line breaks"}`,

"rpt-inv-nms":`You are procurement/materials head writing about non-moving stock action. Based on inventory analysis:
${ctx}
Draft NMS action email to stores/warehouse/suppliers: list of NMS items requiring action, categorized by: return to supplier, scrap, liquidate, transfer. Include deadlines and responsible parties. Clear action-oriented format.
RESPOND ONLY VALID JSON: {"subject":"","body":"email body with \\n for line breaks"}`,

"rpt-inv-cfo":`You are CPO writing to CFO about working capital locked in inventory. Based on:
${ctx}
Draft working capital update: total NMS value, aging profile, recovery plan with timeline and expected recovery %, impact on working capital if action is taken, approvals needed for scrap/write-off. Financial language.
RESPOND ONLY VALID JSON: {"subject":"","body":"email body with \\n for line breaks"}`,

"rpt-ss-warning":`You are CPO writing a formal performance warning to an underperforming supplier. Based on scorecard:
${ctx}
Draft warning letter: cite specific metrics (quality PPM, OTIF%, price competitiveness), state minimum acceptable improvement targets, set 90-day improvement window, mention consequences (volume reduction, de-listing). Formal, documented, but constructive.
RESPOND ONLY VALID JSON: {"subject":"","body":"email body with \\n for line breaks"}`,

"rpt-ss-appreciation":`You are CPO writing to recognize a top-performing supplier. Based on scorecard:
${ctx}
Draft appreciation letter: cite specific performance metrics that stand out, acknowledge consistent quality/delivery, mention potential for increased business, express interest in long-term partnership. Warm, genuine.
RESPOND ONLY VALID JSON: {"subject":"","body":"email body with \\n for line breaks"}`,

"rpt-cat-rfq":`You are a procurement manager launching an RFQ for new suppliers. Based on category opportunity analysis:
${ctx}
Draft market inquiry / RFQ invitation email to potential new suppliers: describe the category and items, mention annual volumes and potential, list required capabilities, request quotation with CBS by deadline. Professional, attractive to suppliers.
RESPOND ONLY VALID JSON: {"subject":"","body":"email body with \\n for line breaks"}`,

"rpt-cat-mgmt":`You are CPO presenting category savings opportunities to management. Based on:
${ctx}
Draft savings pipeline update: ranked list of categories by opportunity size, recommended levers for each, implementation timeline (quick wins vs structural), resource requirements, expected P&L impact, approvals needed.
RESPOND ONLY VALID JSON: {"subject":"","body":"email body with \\n for line breaks"}`,

"rpt-cr-followup":`You are a procurement manager following up on stuck cost reduction initiatives. Based on pipeline tracker:
${ctx}
Draft follow-up email to initiative owners: list initiatives that are delayed or stuck (>30 days overdue), request status update and revised timeline, offer support to unblock, mention next review date. Politely firm.
RESPOND ONLY VALID JSON: {"subject":"","body":"email body with \\n for line breaks"}`,

"rpt-cr-board":`You are CPO preparing a savings update for MD/Board. Based on cost reduction tracker:
${ctx}
Draft board update email: target vs actual savings (₹), pipeline health (identified/in-progress/validated), top 5 completed initiatives with impact, top 5 at-risk initiatives, forecast for remaining year, key decisions needed.
RESPOND ONLY VALID JSON: {"subject":"","body":"email body with \\n for line breaks"}`,

"rpt-sv-finance":`You are CPO requesting finance alignment on savings methodology. Based on savings validation:
${ctx}
Draft email to CFO/finance: present validated savings number vs claimed, explain methodology used, flag items needing finance review, request meeting to align on reporting approach, emphasize hard savings vs avoidance distinction.
RESPOND ONLY VALID JSON: {"subject":"","body":"email body with \\n for line breaks"}`,

"rpt-sv-board":`You are CPO presenting validated procurement savings to board/MD. Based on:
${ctx}
Draft concise board email: total validated savings (₹ crores), hard vs soft vs avoidance breakdown, P&L mapping, year-on-year comparison, top 5 initiatives by impact, procurement ROI calculation. Executive tone.
RESPOND ONLY VALID JSON: {"subject":"","body":"email body with \\n for line breaks"}`,

"rpt-risk-rfq":`You are procurement manager launching dual-sourcing for single-source high-risk items. Based on risk analysis:
${ctx}
Draft RFQ invitation to potential alternate suppliers: describe items currently single-sourced, mention required specifications and quality standards, annual volumes, invite capability presentation + quotation, set timeline for qualification. Position as strategic partnership opportunity.
RESPOND ONLY VALID JSON: {"subject":"","body":"email body with \\n for line breaks"}`,

"rpt-risk-mgmt":`You are CPO escalating supply chain risk to management. Based on risk heat map:
${ctx}
Draft BCP/risk alert: top 5 risks ranked by likelihood x impact, single-source dependencies, geographic concentration, financial vulnerability of key suppliers, mitigation plan with timeline and cost, decisions needed from management.
RESPOND ONLY VALID JSON: {"subject":"","body":"email body with \\n for line breaks"}`
      };
      const prompt=prompts[type];if(!prompt)throw new Error("Unknown type");
      const r=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-5-20250929",max_tokens:2000,temperature:0.3,messages:[{role:"user",content:prompt}]})});
      if(!r.ok)throw new Error("API "+r.status);
      const d=await r.json(),txt=d.content?.map(c=>c.text||"").join("")||"";
      const i1=txt.indexOf("{"),i2=txt.lastIndexOf("}");
  ...
  const RptSmartActions=()=>{
    const acts=RPT_ACTIONS[report.id]||[];
    if(!acts.length)return null;
    return(<div style={{marginTop:20,borderTop:"1px solid var(--s2)",paddingTop:16}}>
      <div style={{fontSize:13,fontWeight:700,marginBottom:10}}>⚡ Smart Actions — What's Next?</div>
      <div style={{display:"grid",gridTemplateColumns:"1fr",gap:8}}>
        {acts.map((a,i)=><div key={i} className="smart-action-card" style={{background:"var(--s1)",border:"1px solid var(--s2)",borderRadius:8,padding:12,cursor:"pointer",transition:"all .15s"}}
          onClick={()=>{if(a.type)genReportEmail(a.type)}}>
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            <span style={{fontSize:18}}>{a.icon}</span>
            <div style={{flex:1}}>
              <div style={{fontSize:12,fontWeight:600}}>{a.label} {a.type&&<span style={{fontSize:9,color:"var(--ac)",fontWeight:400}}>(1 credit)</span>}</div>
      ...
      {emailLoading&&<div className="mod-loading" style={{marginTop:10}}>🤖 AI drafting email... 15 seconds</div>}
      {draftEmail&&<div style={{marginTop:12}}>
        <div style={{background:"var(--bg)",border:"1px solid var(--ac)",borderRadius:10,padding:16}}>
          <div style={{fontSize:10,color:"var(--t3)",marginBottom:2}}>Subject:</div>
          <div style={{fontSize:13,fontWeight:600,marginBottom:10,padding:"6px 10px",background:"var(--s1)",borderRadius:6}}>{draftEmail.subject}</div>
          <div style={{fontSize:10,color:"var(--t3)",marginBottom:2}}>Body:</div>
          <div style={{fontSize:12,lineHeight:1.7,whiteSpace:"pre-wrap",padding:"10px 12px",background:"var(--s1)",borderRadius:6,fontFamily:"DM Sans"}}>{(draftEmail.body||"").replace(/\\n/g,"\n")}</div>
          <div style={{display:"flex",gap:8,marginTop:10}}>
            <button className="btn-pri" style={{fontSize:11}} onClick={()=>{const txt="Subject: "+draftEmail.subject+"\n\n"+draftEmail.body.replace(/\\n/g,"\n");try{const ta=document.createElement("textarea");ta.value=txt;ta.style.cssText="position:fixed;left:-9999px";document.body.appendChild(ta);ta.select();document.execCommand("copy");document.body.removeChild(ta);alert("📋 Copied!")}catch{alert("Copy failed")}}}>📋 Copy Email</button>
            <button className="btn-sec" style={{fontSize:11}} onClick={()=>{const body=draftEmail.body.replace(/\\n/g,"\r\n").replace(/\n/g,"\r\n");window.location.href="mailto:?subject="+encodeURIComponent(draftEmail.subject)+"&body="+encodeURIComponent(body)}}>📧 Open in Mail</button>
          </div>
        </div>
      </div>}
    </div>);
  ...
  if(status==="result"&&result)return(<div style={{maxWidth:750}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14,flexWrap:"wrap",gap:6}}>
      ...
      <div style={{display:"flex",gap:6}}>
        ...
        <button className="btn-sec" style={{fontSize:11,borderColor:"var(--warn)",color:"var(--warn)"}} onClick={()=>{setStatus("upload");setExpertSent(false)}}>👨‍💼 Expert Review</button>
      </div>
    </div>
    <div className="rpt-header"><span style={{fontSize:24}}>{report.icon}</span><div><div style={{fontSize:18,fontWeight:800,fontFamily:"'Bricolage Grotesque'"}}>{result.title||report.name}</div><div style={{fontSize:10,color:"var(--t3)"}}>AI-Generated • {new Date().toLocaleDateString("en-IN")} • CostLens</div></div></div>
    ...
    <RptSmartActions/>
    <div className="rpt-expert-cta"><div style={{fontSize:13,fontWeight:700}}>Need a polished, board-ready version?</div><div style={{fontSize:11,color:"var(--t2)",margin:"4px 0"}}>Our procurement expert reviews this analysis, validates findings, adds market context, and delivers a presentation-ready PDF.</div><button className="btn-sec" style={{borderColor:"var(--warn)",color:"var(--warn)"}} onClick={()=>{setStatus("upload");setExpertSent(false)}}>{report.expertPrice} • Request Expert Review →</button></div>
  </div>);
  return null;
}
  ...
  <section style={{padding:"120px 0 60px",position:"relative",overflow:"hidden"}}>
    ...
    <div className="wrap"><div className="hero-grid-anim" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:50,alignItems:"center"}}>
      <div>
        ...
        <p style={{fontSize:16,color:"var(--t2)",lineHeight:1.7,marginBottom:20,maxWidth:480}}>Zero-based costing for manufacturing procurement. 13 modules, 4 AI commercial tools, 50+ templates. From should-cost to contract analysis — every procurement need in one platform.</p>
        <div style={{display:"flex",gap:10,flexWrap:"wrap"}}><button className="btn-pri" style={{fontSize:15,padding:"12px 28px"}} onClick={onSignup}>Start Free →</button><button className="btn-sec" style={{fontSize:15,padding:"12px 28px"}} onClick={()=>document.getElementById("modules")?.scrollIntoView({behavior:"smooth"})}>See Modules</button></div>
...
class ErrorBoundary extends React.Component{
  ...
  render(){if(this.state.hasError){const e=this.state.error;const stk=this.state.info?.componentStack||"no stack";return React.createElement("div",{style:{padding:20,textAlign:"center",fontFamily:"'DM Sans',sans-serif",color:"#f87171",background:"#070b14",minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:10}},
    React.createElement("div",{style:{fontSize:24}},"⚠️"),
    React.createElement("div",{style:{fontSize:16,fontWeight:700}},"CostLens Debug"),
    React.createElement("div",{style:{fontSize:11,color:"#fbbf24",maxWidth:400,wordBreak:"break-word"}},"ERROR: "+String(e?.message||e||"?")),
    React.createElement("pre",{style:{fontSize:8,color:"#64748b",maxWidth:360,textAlign:"left",background:"#0c1220",padding:8,borderRadius:6,fontFamily:"monospace",whiteSpace:"pre-wrap",maxHeight:160,overflow:"auto",border:"1px solid #1e293b"}},stk),
    React.createElement("button",{onClick:()=>{(async()=>{try{const r=await window.storage?.list("");if(r?.keys)for(const k of r.keys){try{await window.storage.delete(k)}catch{}}}catch{};try{await window.storage?.delete("cl-session");await window.storage?.delete("cl-users");await window.storage?.delete("cl-ver")}catch{}})().finally(()=>this.setState({hasError:false,error:null,info:null}))},style:{background:"#2d7ff9",color:"#fff",border:"none",padding:"8px 20px",borderRadius:8,cursor:"pointer",fontSize:12}},"Clear ALL Data & Retry")
...
function AppInner(){
  ...
  const[lp,setLp]=useState("home");const[showAllFeatures,setShowAllFeatures]=useState(false);const[mobMenu,setMobMenu]=useState(false);
  const[theme,setTheme]=useState("light");
  const toggleTheme=()=>{const t=theme==="dark"?"light":"dark";setTheme(t)};
  const[history,setHistory]=useState([]);const[result,setResult]=useState(null);
  const[plan,setPlan]=useState("free");const[credits,setCredits]=useState(0);const[freeChecks,setFreeChecks]=useState(2);
  const[ebookDetail,setEbookDetail]=useState(null);const[printBook,setPrintBook]=useState(null);
  const[nlEmail,setNlEmail]=useState("");const[nlStatus,setNlStatus]=useState("");

  // Load session from storage on mount
  useEffect(()=>{(async()=>{try{const ver=await dbGet("cl-ver");if(ver!=="10"){await dbSet("cl-ver","10");try{const keys=await window.storage.list();if(keys?.keys){for(const k of keys.keys){if(k!=="cl-ver")try{await window.storage.delete(k)}catch{}}}}catch{};try{await window.storage.delete("cl-session")}catch{};try{await window.storage.delete("cl-users")}catch{}}}catch{};try{const sess=await dbGet("cl-session");if(sess&&typeof sess==="object"&&sess.email){setUser(sess);setPlan(String(sess.plan||"free"));setCredits(Number(sess.credits)||0);try{const h=await dbGet("cl-history-"+sess.email);if(Array.isArray(h))setHistory(h.filter(x=>x&&typeof x.name==="string"))}catch{}}}catch{};setLoading(false)})()},[]);

  const doAuth=async()=>{setAErr("");if(authMode==="register"&&!aName.trim()){setAErr("Name required");return}if(!aEmail.includes("@")){setAErr("Valid email");return}if(aPass.length<4){setAErr("Min 4 chars");return}
    const users=await dbGet("cl-users")||{};
    if(authMode==="register"){if(users[aEmail]){setAErr("Email exists");return}users[aEmail]={name:aName,pass:aPass,plan:"free",credits:0,joined:new Date().toISOString()};await dbSet("cl-users",users);const u={name:aName,email:aEmail,plan:"free",credits:0};setUser(u);await dbSet("cl-session",u);setPlan("free");setCredits(0);setShowAuth(false)}
    else{const u=users[aEmail];if(!u||u.pass!==aPass){setAErr("Invalid credentials");return}const sess={name:u.name,email:aEmail,plan:u.plan||"free",credits:u.credits||0};setUser(sess);await dbSet("cl-session",sess);setPlan(sess.plan);setCredits(sess.credits);const h=await dbGet("cl-history-"+aEmail);if(Array.isArray(h))setHistory(h);setShowAuth(false)}};

  const logout=async()=>{if(user){const users=await dbGet("cl-users")||{};if(users[user.email]){users[user.email].credits=credits;users[user.email].plan=plan;await dbSet("cl-users",users)}}try{await window.storage.delete("cl-session")}catch{}setUser(null);setPage("dashboard");setActiveMod(null);setResult(null)};

  const openMod=id=>{setActiveMod(id);setActiveReport(null);setActiveTool(null);setResult(null);setPage("module")};
  const openReport=id=>{setActiveReport(id);setActiveMod(null);setActiveTool(null);setResult(null);setPage("report")};
  const openTool=id=>{setActiveTool(id);setActiveMod(null);setActiveReport(null);setResult(null);setPage("tool")};
  ...
  const useCredit=()=>{if(credits<=0)return;setCredits(c=>Math.max(0,c-1))};
  ...
  useEffect(()=>{window.__costlens_upgrade=upgrade},[]);
  const cats=[...new Set(MODS.map(m=>m.cat))];

  if(loading)return <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",background:theme==="light"?"#f4f6fa":"#070b14",gap:16}}><CLLogo size={28} white={theme==="dark"}/><div style={{fontSize:13,color:theme==="light"?"#888":"#666",fontFamily:"'DM Sans',sans-serif"}}>Loading...</div></div>;

  // ─── AUTH MODAL ───
  const authModal=showAuth?(<div style={{position:"fixed",inset:0,background:theme==="light"?"rgba(30,41,59,.4)":"rgba(0,0,0,.75)",backdropFilter:"blur(4px)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000}} onClick={()=>setShowAuth(false)}>
    <div style={{background:"var(--s1)",border:"1px solid var(--s3)",borderRadius:16,padding:32,maxWidth:400,width:"90%"}} onClick={e=>e.stopPropagation()}>
      <div style={{textAlign:"center",marginBottom:12}}><div style={{display:"flex",justifyContent:"center"}}><CLLogo size={20} white={theme==="dark"}/></div></div>
      <h3 style={{fontSize:14,marginBottom:10}}>{authMode==="login"?"Sign In":"Create Account"}</h3>
      {aErr&&<div className="mod-err">{aErr}</div>}
      {authMode==="register"&&<div className="mod-field"><label>Name</label><input value={aName} onChange={e=>setAName(e.target.value)} placeholder="Your name"/></div>}
      <div className="mod-field"><label>Email</label><input type="email" value={aEmail} onChange={e=>setAEmail(e.target.value)} placeholder="you@company.com"/></div>
      <div className="mod-field"><label>Password</label><input type="password" value={aPass} onChange={e=>setAPass(e.target.value)} placeholder="••••" onKeyDown={e=>e.key==="Enter"&&doAuth()}/></div>
      <button className="btn-pri" style={{width:"100%",marginTop:8}} onClick={doAuth}>{authMode==="login"?"Sign In →":"Create Account →"}</button>
      <p style={{textAlign:"center",fontSize:11,color:"var(--t3)",marginTop:10}}>{authMode==="login"?<>No account? <span style={{color:"var(--ac)",cursor:"pointer"}} onClick={()=>{setAuthMode("register");setAErr("")}}>Create one</span></>:<>Have account? <span style={{color:"var(--ac)",cursor:"pointer"}} onClick={()=>{setAuthMode("login");setAErr("")}}>Sign in</span></>}</p>
    </div></div>):null;

  // ─── PRINT OVERLAY (ebook PDF) ───
  const printOverlay=printBook?(<div id="print-overlay" style={{position:"fixed",inset:0,background:"#fff",zIndex:2000,overflow:"auto",color:"#1e293b",fontFamily:"Georgia,serif"}}>
    <style>{`@media print{#print-btns{display:none!important}}`}</style>
    <div id="print-btns" style={{position:"sticky",top:0,background:"#fff",borderBottom:"1px solid #e2e8f0",padding:"10px 20px",display:"flex",gap:10,justifyContent:"flex-end",zIndex:1}}>
      <button onClick={()=>setPrintBook(null)} style={{background:"#f1f5f9",border:"1px solid #cbd5e1",borderRadius:8,padding:"8px 20px",cursor:"pointer",fontSize:13,fontFamily:"'DM Sans',sans-serif"}}>✕ Close</button>
      <button onClick={()=>window.print()} style={{background:"#2563eb",color:"#fff",border:"none",borderRadius:8,padding:"8px 20px",cursor:"pointer",fontSize:13,fontWeight:600,fontFamily:"'DM Sans',sans-serif"}}>🖨️ Print / Save PDF</button>
    </div>
    <div style={{maxWidth:650,margin:"30px auto",padding:"0 20px"}}>
      <div style={{textAlign:"center",marginBottom:30}}>
        <div style={{fontSize:11,textTransform:"uppercase",letterSpacing:2,color:"#2563eb",marginBottom:8}}>Procurement Excellence Series • Book {printBook.id}</div>
        <div style={{fontSize:28,fontWeight:700,marginBottom:4,fontFamily:"Georgia,serif"}}>{printBook.title}</div>
        <div style={{fontSize:16,color:"#2563eb",marginBottom:12}}>{printBook.sub}</div>
        <div style={{color:"#64748b",fontSize:13}}>{printBook.pages} pages • PDF • FREE</div>
      </div>
      <div style={{marginBottom:20}}>
        <div style={{fontSize:14,fontWeight:700,marginBottom:12}}>📖 Table of Contents</div>
        <table style={{width:"100%",borderCollapse:"collapse"}}>
          <tbody>{(EBOOK_TOC[printBook.id]||[]).map((ch,i)=><tr key={i}><td style={{color:"#2563eb",fontWeight:700,padding:"8px 12px",verticalAlign:"top",width:50}}>Ch.{i+1}</td><td style={{padding:"8px 12px",borderBottom:"1px solid #e2e8f0",fontSize:14,lineHeight:1.6}}>{ch}</td></tr>)}</tbody>
        </table>
      </div>
      <div style={{marginTop:40,paddingTop:20,borderTop:"2px solid #2563eb",fontSize:12,color:"#64748b",textAlign:"center"}}>CostLens Procurement Excellence Series<br/>© CostLens Platform — Free Educational Resource</div>
    </div>
  </div>):null;

  // ─── EBOOK LIBRARY PAGE ───
  const EbookLibrary=()=>{
    if(ebookDetail){const b=EBOOKS.find(e=>e.id===ebookDetail);const toc=EBOOK_TOC[b.id]||[];
      return(<div>
        <button className="btn-sec" style={{marginBottom:16,fontSize:11}} onClick={()=>setEbookDetail(null)}>← All Ebooks</button>
        <div style={{display:"flex",gap:24,flexWrap:"wrap",alignItems:"flex-start"}}>
          <EbookCover book={b} size="lg"/>
          <div style={{flex:1,minWidth:260}}>
            <div style={{fontSize:8,fontWeight:600,color:b.accent,letterSpacing:1.2,textTransform:"uppercase",marginBottom:4}}>Procurement Excellence Series • Book {b.id}</div>
            <div style={{fontSize:20,fontWeight:800,fontFamily:"'Bricolage Grotesque'",lineHeight:1.2,marginBottom:6}}>{b.title}</div>
            <div style={{fontSize:13,color:b.accent,fontWeight:600,marginBottom:10}}>{b.sub}</div>
            <div style={{display:"flex",gap:12,marginBottom:16,flexWrap:"wrap"}}>
              <span className="rv-badge">{b.pages} pages</span>
              <span className="rv-badge">PDF format</span>
              <span className="rv-badge">FREE download</span>
              {b.tag&&<span className="rv-badge rv-badge-ok">{b.tag}</span>}
            </div>
            <div style={{fontSize:12,color:"var(--t2)",lineHeight:1.7,marginBottom:16}}>A practical, no-fluff guide written by Procurement Experts for procurement professionals who need actionable frameworks, ready-to-use templates, and real-world implementation steps.</div>
            {user?<button className="btn-pri" style={{fontSize:13,padding:"10px 24px"}} onClick={()=>setPrintBook(b)}>📥 Download Free PDF</button>:<button className="btn-pri" style={{fontSize:13,padding:"10px 24px"}} onClick={()=>{setAuthMode("register");setShowAuth(true)}}>Sign Up to Download Free</button>}
          </div>
        </div>
        <div style={{marginTop:24,background:"var(--s1)",border:"1px solid var(--s2)",borderRadius:12,padding:20}}>
          <div style={{fontSize:13,fontWeight:700,marginBottom:14,display:"flex",alignItems:"center",gap:6}}><span style={{fontSize:16}}>📖</span> Table of Contents</div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))",gap:0}}>
            {toc.map((ch,i)=><div key={i} className="toc-item" style={{display:"flex",gap:10,alignItems:"flex-start",padding:"10px 12px",borderBottom:"1px solid var(--s2)"}}>
              <span style={{fontFamily:"'IBM Plex Mono'",fontSize:11,fontWeight:700,color:b.accent,width:24,textAlign:"right",flexShrink:0}}>Ch.{i+1}</span>
              <span style={{fontSize:12,color:"var(--t2)",lineHeight:1.4}}>{ch}</span>
            </div>)}
          </div>
        </div>
        <div style={{marginTop:16,background:"rgba(37,99,235,.04)",border:"1px solid rgba(37,99,235,.15)",borderRadius:10,padding:14,textAlign:"center"}}>
          <div style={{fontSize:11,color:"var(--t2)",marginBottom:8}}>Ready to apply these frameworks?</div>
          <button className="btn-pri" style={{fontSize:11}} onClick={()=>setPage("dashboard")}>🏭 Try CostLens Platform →</button>
        </div>
      </div>);
    }
    return(<div>
      <div style={{fontSize:20,fontWeight:700,marginBottom:4}}>📚 Procurement Excellence Series</div>
      <p style={{fontSize:13,color:"var(--t2)",marginBottom:6}}>10 free ebooks by Procurement Experts — practical frameworks for procurement professionals</p>
      <p style={{fontSize:11,color:"var(--t3)",marginBottom:20}}>💡 Tap any cover to see the table of contents</p>
      ...
      {EBOOKS.map(b=><div key={b.id} style={{background:"var(--s1)",border:"1px solid var(--s2)",borderRadius:12,padding:16,display:"flex",flexDirection:"column",alignItems:"center",gap:10,transition:"all .3s"}}>
        ...
        <div style={{textAlign:"center"}}>
          ...
          <div style={{fontSize:9,color:"var(--t3)",marginBottom:8}}>{b.pages} pages • PDF • FREE</div>
          <div style={{display:"flex",gap:6,justifyContent:"center"}}><button className="btn-sec" style={{fontSize:10,padding:"5px 12px"}} onClick={()=>setEbookDetail(b.id)}>📖 Contents</button>
          {user?<button className="btn-pri" style={{fontSize:10,padding:"5px 12px"}} onClick={()=>setEbookDetail(b.id)}>📥 Download</button>:<button className="btn-pri" style={{fontSize:10,padding:"5px 12px"}} onClick={()=>{setAuthMode("register");setShowAuth(true)}}>📥 Get Free</button>}</div>
        </div>
      </div>)}
    </div>
  </div>);};
  ...
  const PAGES={
    ...
    privacy:()=><FooterPage title="Privacy Policy">
      ...
      <Sh>7. Contact</Sh>
      <P>For privacy inquiries: <strong>privacy@costlens.in</strong></P>
    </FooterPage>,
    ...
    terms:()=><FooterPage title="Terms of Service">
      ...
      <Sh>10. Contact</Sh>
      <P>For legal inquiries: <strong>legal@costlens.in</strong></P>
    </FooterPage>,
    ...
    refund:()=><FooterPage title="Refund Policy">
      ...
      <P>Email <strong>support@costlens.in</strong> with your account email and reason. Refunds are processed within 5-7 business days to the original payment method.</P>
    ...
    contact:()=><FooterPage title="Contact Us">
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,margin:"20px 0"}}>
        {[
          {icon:"📧",title:"General Inquiries",detail:"hello@costlens.in",sub:"We respond within 24 hours"},
          {icon:"🛠️",title:"Technical Support",detail:"support@costlens.in",sub:"Professional: 48 hrs • Team: 24 hrs"},
          {icon:"💼",title:"Enterprise & Partnerships",detail:"enterprise@costlens.in",sub:"Custom plans, API access, bulk licensing"},
          {icon:"📋",title:"Expert Reports",detail:"experts@costlens.in",sub:"On-demand analysis & consulting"},
      ...
      <P>Want to see CostLens in action for your specific use case? We offer free 30-minute demos tailored to your industry — bicycle, automotive, FMCG, industrial, or engineering. Book a slot at <strong>demo@costlens.in</strong></P>
    </FooterPage>,
    ...
    blog:()=><FooterPage title="Blog & Insights">
      ...
      <div style={{background:"rgba(37,99,235,.04)",border:"1px solid rgba(37,99,235,.12)",borderRadius:10,padding:16,marginTop:20,textAlign:"center"}}>
        ...
        <button className="btn-pri" style={{marginTop:10,fontSize:12}} onClick={()=>{setAuthMode("register");setShowAuth(true)}}>Sign Up Free →</button>
      </div>
    </FooterPage>,

    security:()=><FooterPage title="Data Security">
      <P>Your procurement data — supplier pricing, cost breakdowns, spend analytics — is among the most commercially sensitive information in your organization. We treat it that way.</P>
      <Sh>Encryption</Sh>
      <P><strong>In transit:</strong> All data transmitted between your browser and CostLens servers is encrypted using TLS 1.3 with 256-bit encryption. Every API call, file upload, and page load is encrypted.</P>
      <P><strong>At rest:</strong> All stored data is encrypted using AES-256 encryption. Database backups are encrypted with separate keys.</P>
      <Sh>Infrastructure</Sh>
      <P>CostLens runs on enterprise-grade cloud infrastructure with automatic failover, regular security patching, DDoS protection, and 99.9% uptime SLA. All servers are located in India.</P>
      <Sh>Access Controls</Sh>
      <P>Production data access is restricted to essential personnel only. All access is logged and auditable. Multi-factor authentication is enforced for all internal systems.</P>
      <Sh>Document Processing</Sh>
      <P>Uploaded engineering drawings and documents are processed in memory, used for AI extraction, and not permanently stored on disk. Processed data is available only in your account. We do not use your documents to train AI models.</P>
      <Sh>Compliance</Sh>
      <P>CostLens follows SOC 2 Type II security practices. We conduct regular vulnerability assessments and penetration testing. Our data handling practices comply with India's Digital Personal Data Protection Act, 2023.</P>
    </FooterPage>,
    ...
    careers:()=><FooterPage title="Careers at CostLens">
      ...
      <P>Interested? Send your resume and a short note about why CostLens excites you to <strong>careers@costlens.in</strong></P>
    </FooterPage>,
  };

  // ─── LANDING PAGE (not logged in) ───
  if(!user)return(<div className={`CL ${theme}`}>
    <style>{CSS}</style>{authModal}{printOverlay}
    <nav className="nav"><div className="wrap" style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
      <div className="nav-logo" onClick={goHome}><CLLogo size={18} white={theme==="dark"}/></div>
      <button className="mob-menu-btn" onClick={()=>setMobMenu(!mobMenu)} aria-label="Menu">{mobMenu?"✕":"☰"}</button>
      <div className={`nav-links ${mobMenu?"nav-open":""}`}>
        <a className="nav-link" onClick={()=>{setLp("home");setMobMenu(false);setTimeout(()=>document.getElementById("modules")?.scrollIntoView({behavior:"smooth"}),100)}}>Modules</a>
        <a className="nav-link" onClick={()=>{setLp("home");setMobMenu(false);setTimeout(()=>document.getElementById("ebooks")?.scrollIntoView({behavior:"smooth"}),100)}}>Ebooks</a>
        <a className="nav-link" onClick={()=>{setLp("home");setMobMenu(false);setTimeout(()=>document.getElementById("reports")?.scrollIntoView({behavior:"smooth"}),100)}}>Reports</a>
        <a className="nav-link" onClick={()=>{setLp("home");setMobMenu(false);setTimeout(()=>document.getElementById("ai-tools")?.scrollIntoView({behavior:"smooth"}),100)}} style={{color:"var(--ac)"}}>AI Tools ✨</a>
        <a className="nav-link" onClick={()=>{setLp("home");setMobMenu(false);setTimeout(()=>document.getElementById("pricing")?.scrollIntoView({behavior:"smooth"}),100)}}>Pricing</a>
        <button className="btn-pri" style={{fontSize:12,padding:"7px 16px"}} onClick={()=>{setAuthMode("login");setShowAuth(true);setMobMenu(false)}}>Login</button>
        <button className="btn-pri" style={{fontSize:12,padding:"7px 16px",background:"#10b981"}} onClick={()=>{setAuthMode("register");setShowAuth(true);setMobMenu(false)}}>Sign Up Free</button>
      </div>
    </div></nav>
    {/* FOOTER PAGES */}
    ...
    <section id="modules" style={{padding:"60px 0",borderTop:"1px solid var(--s2)"}}><div className="wrap">
      ...
      <div style={{textAlign:"center",marginTop:20}}><button className="btn-pri" style={{fontSize:15,padding:"12px 28px"}} onClick={()=>{setAuthMode("register");setShowAuth(true)}}>Start Using All 13 Modules — Free →</button></div>
    </div></section>
    {/* EBOOKS */}
    <section id="ebooks" style={{padding:"60px 0",borderTop:"1px solid var(--s2)"}}><div className="wrap">
      ...
      <p style={{fontSize:14,color:"var(--t2)",marginBottom:8,maxWidth:600}}>10 free ebooks written by procurement professionals with 25+ years of Indian manufacturing experience. Not theory — real frameworks you can implement Monday morning.</p>
      ...
      <div style={{background:"var(--s1)",border:"1px solid var(--s2)",borderRadius:10,padding:16,marginTop:16,display:"flex",gap:20,alignItems:"center",flexWrap:"wrap",justifyContent:"center"}}>
        {[["1,200+","Downloads"],["4.8/5","Average Rating"],["10","Books (3 Published)"],["25+","Years of Experience"]].map(([v,l],i)=><div key={i} style={{textAlign:"center",minWidth:80}}>
    ...
    <div className="wrap">
      <div style={{textAlign:"center",marginBottom:20}}>
        ...
        <p style={{fontSize:14,color:"var(--t2)",maxWidth:600,margin:"0 auto"}}>Four powerful AI tools that handle the commercial tasks eating up your team's time — contract review, price validation, RFQ comparison, and negotiation strategy.</p>
      </div>

      {/* A DAY IN PROCUREMENT - SCENARIO */}
      <div style={{background:"var(--s1)",border:"1px solid var(--s2)",borderRadius:14,padding:20,marginBottom:28,maxWidth:750,margin:"0 auto 28px"}}>
        ...
        <div style={{textAlign:"center",marginTop:12,fontSize:12,fontWeight:700,color:"var(--ac)"}}>Total time saved: ~2.5 days. Total value protected: ~₹8–12 Lakhs. Credits used: 9.</div>
      </div>
    ...
    <div className="wrap">
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(120px,1fr))",gap:12}}>
        {[
          ...
          {val:"4",label:"AI Commercial Tools",sub:"Contract, RFQ, Price, Negotiate"},
    ...
    <section id="pricing" style={{padding:"60px 0",borderTop:"1px solid var(--s2)"}}><div className="wrap" style={{textAlign:"center"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10,maxWidth:920,margin:"0 auto 30px"}}>
        {[
          {n:"Starter",p:"₹0",d:"Free forever",tag:"",btn:"Start Free",pri:false,sub:"For individual exploration",feat:["All 13 modules + 4 AI tools","50+ templates","2 free Price Checks","Last 5 saved analyses"]},
          {n:"Professional",p:"₹1,999",d:"/month",tag:"MOST POPULAR",btn:"Start Professional →",pri:true,sub:"For procurement managers",feat:["Everything in Starter","50 AI credits/mo","AI reports + commercial tools","Unlimited history + PDF"],ann:"₹19,999/year — save 17%"},
      ...
      <div style={{maxWidth:920,margin:"0 auto",textAlign:"left"}}>
          ...
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
            ...
            <tbody>
              {(()=>{const allRows=[
                ...
                {cat:"AI COMMERCIAL TOOLS"},
                {f:"Price Reasonableness Check",s:"2 free",p:"1 credit",t:"1 credit",e:"✅ Included"},
                {f:"Contract Clause Analyzer",s:"—",p:"3 credits",t:"3 credits",e:"✅ Included"},
                {f:"RFQ Response Comparator",s:"—",p:"3 credits",t:"3 credits",e:"✅ Included"},
                {f:"Negotiation Prep Brief",s:"—",p:"2 credits",t:"2 credits",e:"✅ Included"},
                {cat:"EXPORT & HISTORY"},
                ...
                {f:"10 procurement ebooks",s:"✅",p:"✅",t:"✅",e:"✅"},
        ...
        <div style={{textAlign:"center",marginTop:20}}>
          <button className="btn-pri" style={{fontSize:14,padding:"12px 32px"}} onClick={()=>{setAuthMode("register");setShowAuth(true)}}>Start Free — No Credit Card Required →</button>
    ...
    <footer style={{borderTop:"1px solid var(--s2)",marginTop:20}}>
      {/* CTA Banner */}
      <div style={{background:"linear-gradient(135deg,var(--ac),#7c3aed)",padding:"36px 20px",textAlign:"center"}}>
        ...
        <button className="btn-pri" style={{background:"#fff",color:"var(--ac)",fontSize:14,padding:"12px 32px",fontWeight:700}} onClick={()=>{setAuthMode("register");setShowAuth(true)}}>Start Free — No Credit Card →</button>
      </div>
      {/* Main Footer */}
      <div style={{background:"var(--s1)",padding:"40px 0 20px"}}><div className="wrap">
        <div style={{display:"grid",gridTemplateColumns:"2fr 1fr 1fr 1fr",gap:30,marginBottom:30}}>
          {/* Brand */}
          <div>
            <div style={{marginBottom:8}}><CLLogo size={20} sub white={theme==="dark"}/></div>
          ...
          <div>
            ...
            {["All 13 Modules","Should-Cost Analysis","Tool & Die Costing","TCO Calculator","Spend Analysis","AI Extraction","Free Ebooks"].map((l,i)=>
          ...
          <div>
            ...
            {[{l:"Privacy Policy",p:"privacy"},{l:"Terms of Service",p:"terms"},{l:"Refund Policy",p:"refund"},{l:"Data Security",p:"security"},{l:"GDPR Compliance",p:"privacy"}].map((lk,i)=>
  ...
  return(<div className={`CL ${theme}`}>
    ...
    <div className="layout">
      <div className="sidebar">
        <div className="sb-logo" onClick={()=>{setPage("dashboard");setActiveMod(null);setResult(null)}}><CLLogo size={16} white/></div>
        <div className="sb-nav">
          ...
          {AI_TOOLS.map(t=><div key={t.id} className={`sb-item ${activeTool===t.id?"active":""}`} onClick={()=>openTool(t.id)}><span style={{fontSize:13}}>{t.icon}</span> {t.name}</div>)}
          <div className="sb-cat">Resources</div>
          <div className={`sb-item ${page==="ebooks"?"active":""}`} onClick={()=>{setPage("ebooks");setActiveMod(null);setActiveReport(null);setActiveTool(null)}}>📚 Free Ebooks</div>
        </div>
        <div className="sb-bottom">
          <div className="sb-credits-row">{plan==="pro"?<><div className="sb-cbar"><div style={{width:((credits/50)*100)+"%",height:"100%",background:credits<=5?"var(--err)":"var(--ac)",borderRadius:2}}/></div><span style={{fontSize:9,color:credits<=5?"#f87171":"#96a3b8"}}>{credits} AI</span></>:<span style={{fontSize:9,color:"#667892",padding:"0 4px"}}>Starter — Manual only</span>}</div>
          {plan!=="pro"&&<button className="sb-upgrade-btn" onClick={upgrade}>⚡ Upgrade to Pro</button>}
          {plan==="pro"&&credits<=5&&credits>0&&<div className="sb-low-credits">⚡ {credits} credits left — <span className="upsell-link" onClick={()=>alert("Credits refresh monthly on Pro plan")}>Refill →</span></div>}
          {plan==="pro"&&credits===0&&<div className="sb-low-credits urgent">🔴 0 credits — AI features paused until next cycle</div>}
          <div style={{display:"flex",alignItems:"center",gap:6,padding:"6px 8px"}}><div style={{width:26,height:26,borderRadius:"50%",background:"var(--ac)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,fontWeight:700,color:"#fff"}}>{user.name[0].toUpperCase()}</div><div><div style={{fontSize:11,fontWeight:500}}>{user.name}</div><div style={{fontSize:8,color:"var(--t3)",textTransform:"uppercase"}}>{plan==="pro"?"Professional":"Starter"}</div></div></div>
      ...
      <div className="main"><div className="main-pad">
        {page==="dashboard"&&<div>
          <div style={{fontSize:20,fontWeight:700,marginBottom:16}}>Welcome, {user.name}</div>
          <div className="dash-stats">{[["Analyses",history.length,"var(--ac)"],["AI Credits",credits,""],["Modules","13","var(--ok)"],["AI Tools","4","var(--ac)"],["Plan",plan==="pro"?"PROFESSIONAL":"STARTER",""]].map(([l,v,c],i)=><div key={i} className="dash-stat"><div style={{fontSize:9,color:"var(--t3)",textTransform:"uppercase"}}>{l}</div><div style={{fontFamily:"IBM Plex Mono",fontSize:20,fontWeight:600,color:c||"var(--t1)"}}>{v}</div></div>)}</div>
          <div style={{fontSize:14,fontWeight:600,marginBottom:8}}>Costing Modules</div>
          <div className="mod-grid">{MODS.map(m=><div key={m.id} className="mod-card" onClick={()=>openMod(m.id)}><div style={{position:"absolute",top:0,left:0,right:0,height:3,background:m.color}}/>{m.custom&&plan==="free"&&<span className="mod-ai-badge">AI ⚡</span>}<div style={{fontSize:22,marginBottom:4}}>{m.icon}</div><div style={{fontSize:12,fontWeight:600}}>{m.name}</div><div style={{fontSize:10,color:"var(--t3)"}}>{m.desc}</div></div>)}</div>
          {plan==="free"&&<div className="dash-upgrade-banner" onClick={upgrade}><div className="dash-upgrade-inner"><span style={{fontSize:18}}>🚀</span><div><div style={{fontSize:13,fontWeight:700}}>Unlock AI-Powered Analysis</div><div style={{fontSize:11,color:"var(--t2)"}}>Upload drawings → AI builds process sheets, BOM, and should-cost in 30 seconds. 50 AI credits/month.</div></div><span className="dash-upgrade-price">₹1,999<span style={{fontSize:9,fontWeight:400}}>/mo</span></span></div></div>}
          {/* Ebooks Preview on Dashboard */}
          <div style={{fontSize:14,fontWeight:600,margin:"20px 0 8px"}}>📚 Free Ebooks</div>
          ...
          <span style={{fontSize:11,color:"var(--ac)",cursor:"pointer"}} onClick={()=>{setPage("ebooks");setActiveMod(null)}}>View all 10 ebooks →</span>
          {/* Analysis Reports Preview */}
          <div style={{fontSize:14,fontWeight:600,margin:"20px 0 8px"}}>📊 On-Demand Analysis Reports</div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))",gap:6}}>{REPORTS.slice(0,4).map(r=><div key={r.id} className="mod-card" onClick={()=>openReport(r.id)} style={{borderLeft:`3px solid var(--warn)`}}><div style={{display:"flex",alignItems:"center",gap:6}}><span style={{fontSize:18}}>{r.icon}</span><span style={{fontSize:11,fontWeight:600}}>{r.name}</span></div><div style={{fontSize:10,color:"var(--t3)",marginTop:3}}>{r.desc.slice(0,60)}...</div><div style={{fontSize:9,color:"var(--warn)",marginTop:4}}>{r.credits} AI credits • Expert: {r.expertPrice}</div></div>)}</div>
          <span style={{fontSize:11,color:"var(--warn)",cursor:"pointer"}} onClick={()=>{setPage("reports");setActiveMod(null);setActiveReport(null)}}>View all 8 analysis reports →</span>
          {/* AI Commercial Tools Preview */}
          <div style={{fontSize:14,fontWeight:600,margin:"20px 0 8px"}}>📑 AI Commercial Tools <span style={{fontSize:10,fontWeight:400,color:"var(--ac)",background:"rgba(59,130,246,.08)",padding:"2px 8px",borderRadius:10,marginLeft:6}}>NEW</span></div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))",gap:6}}>{AI_TOOLS.map(t=><div key={t.id} className="mod-card" onClick={()=>openTool(t.id)} style={{borderLeft:`3px solid var(--ac)`}}><div style={{display:"flex",alignItems:"center",gap:6}}><span style={{fontSize:18}}>{t.icon}</span><span style={{fontSize:11,fontWeight:600}}>{t.name}</span></div><div style={{fontSize:10,color:"var(--t3)",marginTop:3}}>{t.desc.slice(0,65)}...</div><div style={{fontSize:9,color:"var(--ac)",marginTop:4}}>{t.credits} AI credit{t.credits>1?"s":""} • {t.needsFile?"Upload files":"Form entry"}</div></div>)}</div>
          <span style={{fontSize:11,color:"var(--ac)",cursor:"pointer"}} onClick={()=>{setPage("tools");setActiveMod(null);setActiveTool(null)}}>View all 4 AI tools →</span>
          {history.length>0&&<><div style={{fontSize:14,fontWeight:600,margin:"20px 0 8px"}}>Recent Analyses</div>
            {history.slice(0,8).map(h=>{const m=MODS.find(x=>x.id===h.module);return <div key={h.id} className="hist-item" onClick={()=>{setResult(h);setPage("result")}}><div><div style={{fontSize:12,fontWeight:500}}>{m?.icon} {h.name}</div><div style={{fontSize:9,color:"var(--t3)"}}>{m?.name} • {h.date}</div></div><div style={{fontFamily:"IBM Plex Mono",fontWeight:600,color:"var(--ac)",fontSize:12}}>{typeof h.resultVal==="object"?"₹"+JSON.stringify(h.resultVal):h.resultVal}</div></div>})}</>}
        </div>}
        {page==="ebooks"&&<EbookLibrary/>}
        {page==="reports"&&<div>
        ...
        {page==="tools"&&<div>
          ...
          <p style={{fontSize:13,color:"var(--t2)",marginBottom:16}}>AI-powered tools for contract analysis, price validation, RFQ comparison, and negotiation strategy</p>
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))",gap:10}}>{AI_TOOLS.map(t=><div key={t.id} className="rpt-card" onClick={()=>openTool(t.id)}>
            <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}><span style={{fontSize:22}}>{t.icon}</span><div style={{fontSize:14,fontWeight:700}}>{t.name}</div></div>
            <div style={{fontSize:11,color:"var(--t2)",marginBottom:8,lineHeight:1.5}}>{t.desc}</div>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <div><div style={{fontSize:9,color:"var(--t3)"}}>AI Credits</div><div style={{fontFamily:"IBM Plex Mono",fontSize:12,fontWeight:600,color:"var(--ac)"}}>{t.credits} credit{t.credits>1?"s":""}</div></div>
              <div style={{textAlign:"right"}}><div style={{fontSize:9,color:"var(--t3)"}}>Input</div><div style={{fontSize:11,fontWeight:600}}>{t.needsFile?"File Upload":"Form Entry"}</div></div>
            </div>
          </div>)}</div>
        </div>}
        {page==="tool"&&activeTool&&(()=>{const t=AI_TOOLS.find(x=>x.id===activeTool);return t?<AIToolRunner tool={t} credits={credits} useCredit={useCredit} user={user} plan={plan} freeChecks={freeChecks} useFreeCheck={()=>setFreeChecks(p=>Math.max(0,p-1))} onBack={()=>{setPage("tools");setActiveTool(null)}}/>:null})()}
        {page==="module"&&activeMod&&<div>
          ...
          {!["should-cost","tool-cost"].includes(activeMod)&&MCFG[activeMod]&&<GenericModule moduleId={activeMod} onResult={handleResult} credits={credits} useCredit={useCredit}/>}
        </div>}
...
.nav-logo{font-family:'Bricolage Grotesque',sans-serif;font-weight:800;font-size:18px;cursor:pointer}.nav-link{color:var(--t2);font-size:12px;font-weight:500;cursor:pointer;text-decoration:none}.nav-link:hover{color:var(--t1)}
...
.sb-logo{padding:12px;font-family:'Bricolage Grotesque',sans-serif;font-weight:800;font-size:16px;border-bottom:1px solid var(--s2);cursor:pointer}
...
.main{flex:1;overflow-y:auto}.main-pad{padding:20px 28px;max-width:1000px}
...
.rpt-sev.high{background:rgba(244,63,94,.12);color:var(--err)}.rpt-sev.critical{background:rgba(244,63,94,.18);color:var(--err)}.rpt-sev.medium{background:rgba(245,158,11,.12);color:var(--warn)}.rpt-sev.low{background:rgba(16,185,129,.12);color:var(--ok)}
.smart-action-card:hover{border-color:var(--ac)!important;background:rgba(59,130,246,.04)!important;transform:translateX(4px)}
...
.rpt-snippet{background:linear-gradient(135deg,rgba(37,99,235,.04),rgba(37,99,235,.01));border:1px solid rgba(37,99,235,.15);border-radius:12px;padding:16px 18px;margin-top:16px}
.snip-section{margin-bottom:14px}.snip-section:last-of-type{margin-bottom:6px}
.snip-title{font-size:10px;font-weight:700;color:var(--t3);text-transform:uppercase;letter-spacing:.5px;margin-bottom:6px;padding-bottom:4px;border-bottom:1px solid var(--s2)}
.snip-es{display:flex;gap:8px;align-items:flex-start;padding:4px 0;font-size:11px;color:var(--t2);line-height:1.5}
.snip-num{background:var(--ac);color:#fff;width:18px;height:18px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:8px;font-weight:700;flex-shrink:0;margin-top:2px}
.snip-mg{display:grid;grid-template-columns:repeat(4,1fr);gap:6px}@media(max-width:600px){.snip-mg{grid-template-columns:repeat(2,1fr)}}
.snip-mc{background:var(--s1);border:1px solid var(--s2);border-radius:6px;padding:8px 10px}
.snip-mc-l{font-size:8px;text-transform:uppercase;letter-spacing:.4px;color:var(--t3)}
.snip-mc-v{font-family:'IBM Plex Mono',monospace;font-size:14px;font-weight:700;margin:1px 0}
.snip-mc-t{font-size:8px;font-weight:600}.snip-mc-t.up{color:var(--err)}.snip-mc-t.down{color:var(--ok)}.snip-mc-t.stable{color:var(--t3)}
.snip-finding{display:flex;gap:8px;align-items:flex-start;padding:6px 0;border-bottom:1px solid var(--s2)}.snip-finding:last-child{border-bottom:none}
.snip-f-rank{font-family:'IBM Plex Mono',monospace;font-weight:700;color:var(--ac);font-size:11px;flex-shrink:0;width:22px}
.snip-f-text{font-size:11px;font-weight:500}.snip-f-action{font-size:9px;color:var(--ac);margin-top:2px}
.snip-f-impact{font-family:'IBM Plex Mono',monospace;font-size:10px;font-weight:700;color:var(--ok);white-space:nowrap;flex-shrink:0}
.snip-risk{display:flex;gap:8px;align-items:center;padding:4px 0;font-size:11px;color:var(--t2)}
.snip-sev{display:inline-block;padding:1px 7px;border-radius:8px;font-size:8px;font-weight:700;text-transform:uppercase;flex-shrink:0}
.snip-sev.high{background:rgba(239,68,68,.12);color:#f87171;border:1px solid rgba(239,68,68,.25)}
.snip-sev.medium{background:rgba(245,158,11,.12);color:#fbbf24;border:1px solid rgba(245,158,11,.25)}
.snip-sev.low{background:rgba(16,185,129,.12);color:#34d399;border:1px solid rgba(16,185,129,.25)}
/* ─── UPGRADE UPSELL SYSTEM ─── */
.upsell-bar{padding:10px 14px;border-radius:8px;font-size:11px;cursor:pointer;transition:all .2s;margin-bottom:10px;line-height:1.5}
.upsell-bar:hover{transform:translateY(-1px)}
.ai-nudge{background:linear-gradient(135deg,rgba(37,99,235,.08),rgba(99,102,241,.06));border:1px solid rgba(37,99,235,.2);color:var(--t2)}
.ai-nudge:hover{border-color:rgba(37,99,235,.4);background:linear-gradient(135deg,rgba(37,99,235,.12),rgba(99,102,241,.08))}
.upgrade-nudge{background:linear-gradient(135deg,rgba(245,158,11,.08),rgba(249,115,22,.06));border:1px solid rgba(245,158,11,.25);color:var(--t2)}
.upgrade-nudge:hover{border-color:rgba(245,158,11,.5);background:linear-gradient(135deg,rgba(245,158,11,.14),rgba(249,115,22,.10))}
.upsell-link{color:var(--ac);font-weight:600;cursor:pointer;text-decoration:underline;text-underline-offset:2px}
.upsell-inline{font-size:10px;color:var(--t3);padding:8px 12px;margin-top:8px;background:rgba(37,99,235,.04);border:1px dashed rgba(37,99,235,.15);border-radius:6px;line-height:1.5}
.upsell-btn{font-family:'DM Sans',sans-serif;background:linear-gradient(135deg,#f59e0b,#f97316);color:#fff;border:none;border-radius:8px;padding:8px 18px;font-size:12px;font-weight:600;cursor:pointer;transition:all .2s;display:inline-flex;align-items:center;gap:4px}
.upsell-btn:hover{transform:translateY(-1px);box-shadow:0 4px 12px rgba(245,158,11,.3)}
.upsell-locked{background:var(--s1);border:1px solid var(--s2);border-radius:8px;padding:12px;margin-top:14px;position:relative;cursor:pointer;transition:border-color .2s}
.upsell-locked:hover{border-color:rgba(245,158,11,.4)}
.upsell-blur{filter:blur(4px);user-select:none;pointer-events:none}
.upsell-overlay{position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;background:rgba(2,6,23,.6);border-radius:8px;font-size:11px;color:var(--t2);text-align:center;line-height:1.6;backdrop-filter:blur(2px)}
.upsell-volume-overlay{position:absolute;top:0;right:0;bottom:0;width:75%;display:flex;align-items:center;justify-content:center;background:linear-gradient(90deg,transparent,rgba(2,6,23,.85) 25%);font-size:11px;color:var(--t2);cursor:pointer;border-radius:0 8px 8px 0;padding:0 16px;text-align:center}
.result-upgrade-cta{background:linear-gradient(135deg,rgba(245,158,11,.08),rgba(249,115,22,.04));border:1px solid rgba(245,158,11,.2);border-radius:8px;padding:12px 16px;margin-top:14px;font-size:11px;color:var(--t2);cursor:pointer;transition:all .2s;text-align:center;line-height:1.5}
.result-upgrade-cta:hover{border-color:rgba(245,158,11,.5);transform:translateY(-1px);box-shadow:0 4px 16px rgba(245,158,11,.1)}
.sb-upgrade-btn{width:100%;padding:6px 0;background:linear-gradient(135deg,#f59e0b,#f97316);color:#fff;border:none;border-radius:6px;font-family:'DM Sans',sans-serif;font-size:10px;font-weight:700;cursor:pointer;transition:all .2s;margin:4px 8px;letter-spacing:.3px}
.sb-upgrade-btn:hover{filter:brightness(1.1);transform:translateY(-1px)}
.sb-low-credits{font-size:9px;padding:3px 8px;color:#fbbf24;line-height:1.4}
.sb-low-credits.urgent{color:#f87171;font-weight:600}
.mod-ai-badge{position:absolute;top:6px;right:6px;background:linear-gradient(135deg,#3b82f6,#6366f1);color:#fff;font-size:7px;font-weight:700;padding:2px 6px;border-radius:8px;letter-spacing:.5px;z-index:1}
.dash-upgrade-banner{margin-top:14px;cursor:pointer;transition:all .2s}
.dash-upgrade-banner:hover{transform:translateY(-2px)}
.dash-upgrade-inner{background:linear-gradient(135deg,rgba(245,158,11,.08),rgba(249,115,22,.04));border:1px solid rgba(245,158,11,.2);border-radius:12px;padding:16px 20px;display:flex;align-items:center;gap:14px}
.dash-upgrade-price{font-family:'IBM Plex Mono',monospace;font-size:18px;font-weight:700;color:var(--warn);white-space:nowrap;flex-shrink:0}
/* ─── EBOOK FLIP CARD ─── */
.ebook-flip{cursor:pointer}
.ebook-flip-inner{position:relative;transition:transform .6s cubic-bezier(.4,.2,.2,1);transform-style:preserve-3d}
.ebook-flip:hover .ebook-flip-inner{transform:rotateY(180deg)}
.ebook-face{position:absolute;top:0;left:0;backface-visibility:hidden;-webkit-backface-visibility:hidden;overflow:hidden}
.ebook-front{z-index:2}
.ebook-back{transform:rotateY(180deg)}
.ebook-hover-hint{position:absolute;bottom:6px;right:8px;font-size:7px;color:rgba(255,255,255,.3);opacity:1;transition:opacity .2s}
.ebook-flip:hover .ebook-hover-hint{opacity:0}
@media(max-width:768px){.rpt-paths{grid-template-columns:1fr}.rpt-metrics-grid{grid-template-columns:repeat(2,1fr)}.dash-upgrade-inner{flex-direction:column;text-align:center}}
.mob-menu-btn{display:none;background:none;border:1px solid var(--s3);border-radius:8px;padding:6px 12px;cursor:pointer;font-size:18px;color:var(--t1);line-height:1}
.nav-links{display:flex;gap:20px;align-items:center}
@media(max-width:768px){
.CL{overflow-x:hidden;width:100vw}
.mob-menu-btn{display:block}
.nav-links{display:none;position:absolute;top:100%;left:0;right:0;background:var(--bg);border-bottom:1px solid var(--s2);padding:16px 24px;flex-direction:column;gap:12px;align-items:stretch;box-shadow:0 10px 30px rgba(0,0,0,.2);z-index:99}
.nav-links.nav-open{display:flex}
.nav-links .nav-link{font-size:14px;padding:8px 0;border-bottom:1px solid var(--s2)}
.nav-links .btn-pri{text-align:center}
.nav{position:relative}
.wrap{padding:0 16px}
.hero-grid-anim{padding:0 16px}
.mob-back{display:flex!important}
.sidebar{display:none}
.route-grid{grid-template-columns:1fr 1fr}
}
.mob-back{display:none;align-items:center;gap:6px;background:none;border:none;color:var(--ac);font-size:13px;cursor:pointer;font-family:'DM Sans',sans-serif;padding:10px 0;font-weight:600}
`;
